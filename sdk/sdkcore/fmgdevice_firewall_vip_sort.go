...
// sortFirewallVipItem contains the parameters for each item
type sortFirewallVipItem struct {
	name string
}

func getEntryListFirewallVip(c *FortiSDKClient, inputModel *SortInputModel) (itemList []sortFirewallVipItem, err error) {
	path := "/pm/config/device/{device}/vdom/{vdom}/firewall/vip"
	...
	if err == nil {
		...
		var members []sortFirewallVipItem
...
func bEntryListSortedFirewallVip(itemList []sortFirewallVipItem, inputModel *SortInputModel) (bsorted bool) {
...
func moveAfterFirewallVip(idbefore, idafter string, c *FortiSDKClient, inputModel *SortInputModel) (err error) {
	path := "/pm/config/device/{device}/vdom/{vdom}/firewall/vip/"
	path, err = replaceParaWithValue(path, inputModel.URLParams)
...
func sortEntryListFirewallVip(itemList []sortFirewallVipItem, c *FortiSDKClient, inputModel *SortInputModel) (err error) {
	sortby := inputModel.SortBy
	sortdirection := inputModel.SortDirection
	manual_order := inputModel.ManualOrder
	var targetItemOrder []sortFirewallVipItem
	if sortby == "name" {
		if sortdirection == "ascending" {
			...
			curItemMap := make(map[string]sortFirewallVipItem)
	...
	for i := 0; i < len(targetItemOrder)-1; i++ {
		err = moveAfterFirewallVip(targetItemOrder[i+1].name, targetItemOrder[i].name, c, inputModel)
		if err != nil {
...
// CreateUpdateFirewallVipSort API operation for FortiManager to sort the firewall policies.
// Returns error for service API and SDK errors.
func (c *FortiSDKClient) CreateUpdateFirewallVipSort(inputModel *SortInputModel) (err error) {
	itemList, err := getEntryListFirewallVip(c, inputModel)
	...
	bsorted := bEntryListSortedFirewallVip(itemList, inputModel)
	if bsorted == true {
		return
	}

	err = sortEntryListFirewallVip(itemList, c, inputModel)
	if err != nil {
...
// ReadFirewallVipSort API operation for FortiManager to read the firewall policies sort results
// Returns sort status
// Returns error for service API and SDK errors.
func (c *FortiSDKClient) ReadFirewallVipSort(inputModel *SortInputModel) (sorted bool, itemMapList []interface{}, err error) {
	itemList, err := getEntryListFirewallVip(c, inputModel)
	if err != nil {

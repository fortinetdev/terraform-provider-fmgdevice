...
// sortCasbProfileItem contains the parameters for each item
type sortCasbProfileItem struct {
	name string
}

func getEntryListCasbProfile(c *FortiSDKClient, inputModel *SortInputModel) (itemList []sortCasbProfileItem, err error) {
	path := "/pm/config/device/{device}/vdom/{vdom}/casb/profile"
	...
	if err == nil {
		...
		var members []sortCasbProfileItem
...
func bEntryListSortedCasbProfile(itemList []sortCasbProfileItem, inputModel *SortInputModel) (bsorted bool) {
...
func moveAfterCasbProfile(idbefore, idafter string, c *FortiSDKClient, inputModel *SortInputModel) (err error) {
	path := "/pm/config/device/{device}/vdom/{vdom}/casb/profile/"
	path, err = replaceParaWithValue(path, inputModel.URLParams)
...
func sortEntryListCasbProfile(itemList []sortCasbProfileItem, c *FortiSDKClient, inputModel *SortInputModel) (err error) {
	sortby := inputModel.SortBy
	sortdirection := inputModel.SortDirection
	manual_order := inputModel.ManualOrder
	var targetItemOrder []sortCasbProfileItem
	if sortby == "name" {
		if sortdirection == "ascending" {
			...
			curItemMap := make(map[string]sortCasbProfileItem)
	...
	for i := 0; i < len(targetItemOrder)-1; i++ {
		err = moveAfterCasbProfile(targetItemOrder[i+1].name, targetItemOrder[i].name, c, inputModel)
		if err != nil {
...
// CreateUpdateCasbProfileSort API operation for FortiManager to sort the firewall policies.
// Returns error for service API and SDK errors.
func (c *FortiSDKClient) CreateUpdateCasbProfileSort(inputModel *SortInputModel) (err error) {
	itemList, err := getEntryListCasbProfile(c, inputModel)
	...
	bsorted := bEntryListSortedCasbProfile(itemList, inputModel)
	if bsorted == true {
		return
	}

	err = sortEntryListCasbProfile(itemList, c, inputModel)
	if err != nil {
...
// ReadCasbProfileSort API operation for FortiManager to read the firewall policies sort results
// Returns sort status
// Returns error for service API and SDK errors.
func (c *FortiSDKClient) ReadCasbProfileSort(inputModel *SortInputModel) (sorted bool, itemMapList []interface{}, err error) {
	itemList, err := getEntryListCasbProfile(c, inputModel)
	if err != nil {

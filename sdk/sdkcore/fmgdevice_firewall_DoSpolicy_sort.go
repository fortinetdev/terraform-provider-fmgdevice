...
// sortFirewallDosPolicyItem contains the parameters for each item
type sortFirewallDosPolicyItem struct {
	policyid int
}

func getEntryListFirewallDosPolicy(c *FortiSDKClient, inputModel *SortInputModel) (itemList []sortFirewallDosPolicyItem, err error) {
	path := "/pm/config/device/{device}/vdom/{vdom}/firewall/DoS-policy"
	...
	if err == nil {
		...
		var members []sortFirewallDosPolicyItem
		for _, v := range listTmp {
			c := v.(map[string]interface{})

			members = append(members,
				sortFirewallDosPolicyItem{
...
func bEntryListSortedFirewallDosPolicy(itemList []sortFirewallDosPolicyItem, inputModel *SortInputModel) (bsorted bool) {
...
func moveAfterFirewallDosPolicy(idbefore, idafter int, c *FortiSDKClient, inputModel *SortInputModel) (err error) {
...
func sortEntryListFirewallDosPolicy(itemList []sortFirewallDosPolicyItem, c *FortiSDKClient, inputModel *SortInputModel) (err error) {
	sortby := inputModel.SortBy
	sortdirection := inputModel.SortDirection
	manual_order := inputModel.ManualOrder
	var targetItemOrder []sortFirewallDosPolicyItem
	if sortby == "policyid" {
		if sortdirection == "ascending" {
			...
			curItemMap := make(map[string]sortFirewallDosPolicyItem)
	...
	for i := 0; i < len(targetItemOrder)-1; i++ {
		err = moveAfterFirewallDosPolicy(targetItemOrder[i+1].policyid, targetItemOrder[i].policyid, c, inputModel)
		if err != nil {
...
// CreateUpdateFirewallDosPolicySort API operation for FortiManager to sort the firewall policies.
// Returns error for service API and SDK errors.
func (c *FortiSDKClient) CreateUpdateFirewallDosPolicySort(inputModel *SortInputModel) (err error) {
	itemList, err := getEntryListFirewallDosPolicy(c, inputModel)
	...
	bsorted := bEntryListSortedFirewallDosPolicy(itemList, inputModel)
	if bsorted == true {
		return
	}

	err = sortEntryListFirewallDosPolicy(itemList, c, inputModel)
	if err != nil {
...
// ReadFirewallDosPolicySort API operation for FortiManager to read the firewall policies sort results
// Returns sort status
// Returns error for service API and SDK errors.
func (c *FortiSDKClient) ReadFirewallDosPolicySort(inputModel *SortInputModel) (sorted bool, itemMapList []interface{}, err error) {
	itemList, err := getEntryListFirewallDosPolicy(c, inputModel)
	if err != nil {

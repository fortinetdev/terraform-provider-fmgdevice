...
// sortFirewallLocalInPolicy6Item contains the parameters for each item
type sortFirewallLocalInPolicy6Item struct {
	policyid int
}

func getEntryListFirewallLocalInPolicy6(c *FortiSDKClient, inputModel *SortInputModel) (itemList []sortFirewallLocalInPolicy6Item, err error) {
	path := "/pm/config/device/{device}/vdom/{vdom}/firewall/local-in-policy6"
...
func bEntryListSortedFirewallLocalInPolicy6(itemList []sortFirewallLocalInPolicy6Item, inputModel *SortInputModel) (bsorted bool) {
...
func moveAfterFirewallLocalInPolicy6(idbefore, idafter int, c *FortiSDKClient, inputModel *SortInputModel) (err error) {
...
func sortEntryListFirewallLocalInPolicy6(itemList []sortFirewallLocalInPolicy6Item, c *FortiSDKClient, inputModel *SortInputModel) (err error) {
	sortby := inputModel.SortBy
	sortdirection := inputModel.SortDirection
	manual_order := inputModel.ManualOrder
	...
	for i := 0; i < len(targetItemOrder)-1; i++ {
		err = moveAfterFirewallLocalInPolicy6(targetItemOrder[i+1].policyid, targetItemOrder[i].policyid, c, inputModel)
		if err != nil {
...
// CreateUpdateFirewallLocalInPolicy6Sort API operation for FortiManager to sort the firewall policies.
// Returns error for service API and SDK errors.
func (c *FortiSDKClient) CreateUpdateFirewallLocalInPolicy6Sort(inputModel *SortInputModel) (err error) {
	itemList, err := getEntryListFirewallLocalInPolicy6(c, inputModel)
	...
	bsorted := bEntryListSortedFirewallLocalInPolicy6(itemList, inputModel)
	if bsorted == true {
		return
	}

	err = sortEntryListFirewallLocalInPolicy6(itemList, c, inputModel)
	if err != nil {
...
// ReadFirewallLocalInPolicy6Sort API operation for FortiManager to read the firewall policies sort results
// Returns sort status
// Returns error for service API and SDK errors.
func (c *FortiSDKClient) ReadFirewallLocalInPolicy6Sort(inputModel *SortInputModel) (sorted bool, itemMapList []interface{}, err error) {
	itemList, err := getEntryListFirewallLocalInPolicy6(c, inputModel)
	if err != nil {

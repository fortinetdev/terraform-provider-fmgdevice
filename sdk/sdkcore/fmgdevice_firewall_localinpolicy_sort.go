...
// sortFirewallLocalInPolicyItem contains the parameters for each item
type sortFirewallLocalInPolicyItem struct {
	policyid int
}

func getEntryListFirewallLocalInPolicy(c *FortiSDKClient, inputModel *SortInputModel) (itemList []sortFirewallLocalInPolicyItem, err error) {
	path := "/pm/config/device/{device}/vdom/{vdom}/firewall/local-in-policy"
...
func bEntryListSortedFirewallLocalInPolicy(itemList []sortFirewallLocalInPolicyItem, inputModel *SortInputModel) (bsorted bool) {
...
func moveAfterFirewallLocalInPolicy(idbefore, idafter int, c *FortiSDKClient, inputModel *SortInputModel) (err error) {
...
func sortEntryListFirewallLocalInPolicy(itemList []sortFirewallLocalInPolicyItem, c *FortiSDKClient, inputModel *SortInputModel) (err error) {
	sortby := inputModel.SortBy
	sortdirection := inputModel.SortDirection
	manual_order := inputModel.ManualOrder
	...
	for i := 0; i < len(targetItemOrder)-1; i++ {
		err = moveAfterFirewallLocalInPolicy(targetItemOrder[i+1].policyid, targetItemOrder[i].policyid, c, inputModel)
		if err != nil {
...
// CreateUpdateFirewallLocalInPolicySort API operation for FortiManager to sort the firewall policies.
// Returns error for service API and SDK errors.
func (c *FortiSDKClient) CreateUpdateFirewallLocalInPolicySort(inputModel *SortInputModel) (err error) {
	itemList, err := getEntryListFirewallLocalInPolicy(c, inputModel)
	...
	bsorted := bEntryListSortedFirewallLocalInPolicy(itemList, inputModel)
	if bsorted == true {
		return
	}

	err = sortEntryListFirewallLocalInPolicy(itemList, c, inputModel)
	if err != nil {
...
// ReadFirewallLocalInPolicySort API operation for FortiManager to read the firewall policies sort results
// Returns sort status
// Returns error for service API and SDK errors.
func (c *FortiSDKClient) ReadFirewallLocalInPolicySort(inputModel *SortInputModel) (sorted bool, itemMapList []interface{}, err error) {
	itemList, err := getEntryListFirewallLocalInPolicy(c, inputModel)
	if err != nil {

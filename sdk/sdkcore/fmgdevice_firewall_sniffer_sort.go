...
// sortFirewallSnifferItem contains the parameters for each item
type sortFirewallSnifferItem struct {
	id int
}

func getEntryListFirewallSniffer(c *FortiSDKClient, inputModel *SortInputModel) (itemList []sortFirewallSnifferItem, err error) {
	...
	if err == nil {
		...
		var members []sortFirewallSnifferItem
		for _, v := range listTmp {
			c := v.(map[string]interface{})

			members = append(members,
				sortFirewallSnifferItem{
					id: int(c["id"].(float64)),
				})
		}
...
func bEntryListSortedFirewallSniffer(itemList []sortFirewallSnifferItem, inputModel *SortInputModel) (bsorted bool) {
...
func moveAfterFirewallSniffer(idbefore, idafter int, c *FortiSDKClient, inputModel *SortInputModel) (err error) {
...
func sortEntryListFirewallSniffer(itemList []sortFirewallSnifferItem, c *FortiSDKClient, inputModel *SortInputModel) (err error) {
	sortby := inputModel.SortBy
	sortdirection := inputModel.SortDirection
	manual_order := inputModel.ManualOrder
	var targetItemOrder []sortFirewallSnifferItem
	if sortby == "id" {
		if sortdirection == "ascending" {
			...
			curItemMap := make(map[string]sortFirewallSnifferItem)
	...
	for i := 0; i < len(targetItemOrder)-1; i++ {
		err = moveAfterFirewallSniffer(targetItemOrder[i+1].id, targetItemOrder[i].id, c, inputModel)
		if err != nil {
...
// CreateUpdateFirewallSnifferSort API operation for FortiManager to sort the firewall policies.
// Returns error for service API and SDK errors.
func (c *FortiSDKClient) CreateUpdateFirewallSnifferSort(inputModel *SortInputModel) (err error) {
	itemList, err := getEntryListFirewallSniffer(c, inputModel)
	...
	bsorted := bEntryListSortedFirewallSniffer(itemList, inputModel)
	if bsorted == true {
		return
	}

	err = sortEntryListFirewallSniffer(itemList, c, inputModel)
	if err != nil {
...
// ReadFirewallSnifferSort API operation for FortiManager to read the firewall policies sort results
// Returns sort status
// Returns error for service API and SDK errors.
func (c *FortiSDKClient) ReadFirewallSnifferSort(inputModel *SortInputModel) (sorted bool, itemMapList []interface{}, err error) {
	itemList, err := getEntryListFirewallSniffer(c, inputModel)
	if err != nil {

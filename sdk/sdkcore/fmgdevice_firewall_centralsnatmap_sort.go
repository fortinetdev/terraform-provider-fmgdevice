...
// sortFirewallCentralSnatMapItem contains the parameters for each item
type sortFirewallCentralSnatMapItem struct {
	policyid int
}

func getEntryListFirewallCentralSnatMap(c *FortiSDKClient, inputModel *SortInputModel) (itemList []sortFirewallCentralSnatMapItem, err error) {
	path := "/pm/config/device/{device}/vdom/{vdom}/firewall/central-snat-map"
	...
	if err == nil {
		...
		var members []sortFirewallCentralSnatMapItem
...
func bEntryListSortedFirewallCentralSnatMap(itemList []sortFirewallCentralSnatMapItem, inputModel *SortInputModel) (bsorted bool) {
...
func moveAfterFirewallCentralSnatMap(idbefore, idafter int, c *FortiSDKClient, inputModel *SortInputModel) (err error) {
...
func sortEntryListFirewallCentralSnatMap(itemList []sortFirewallCentralSnatMapItem, c *FortiSDKClient, inputModel *SortInputModel) (err error) {
	sortby := inputModel.SortBy
	sortdirection := inputModel.SortDirection
	manual_order := inputModel.ManualOrder
	var targetItemOrder []sortFirewallCentralSnatMapItem
	if sortby == "policyid" {
		if sortdirection == "ascending" {
	...
	for i := 0; i < len(targetItemOrder)-1; i++ {
		err = moveAfterFirewallCentralSnatMap(targetItemOrder[i+1].policyid, targetItemOrder[i].policyid, c, inputModel)
		if err != nil {
...
// CreateUpdateFirewallCentralSnatMapSort API operation for FortiManager to sort the firewall policies.
// Returns error for service API and SDK errors.
func (c *FortiSDKClient) CreateUpdateFirewallCentralSnatMapSort(inputModel *SortInputModel) (err error) {
	itemList, err := getEntryListFirewallCentralSnatMap(c, inputModel)
	...
	bsorted := bEntryListSortedFirewallCentralSnatMap(itemList, inputModel)
	if bsorted == true {
		return
	}

	err = sortEntryListFirewallCentralSnatMap(itemList, c, inputModel)
	if err != nil {
...
// ReadFirewallCentralSnatMapSort API operation for FortiManager to read the firewall policies sort results
// Returns sort status
// Returns error for service API and SDK errors.
func (c *FortiSDKClient) ReadFirewallCentralSnatMapSort(inputModel *SortInputModel) (sorted bool, itemMapList []interface{}, err error) {
	itemList, err := getEntryListFirewallCentralSnatMap(c, inputModel)
	if err != nil {

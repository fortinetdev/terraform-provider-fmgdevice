...
// sortLlmServerItem contains the parameters for each item
type sortLlmServerItem struct {
	name string
}

func getEntryListLlmServer(c *FortiSDKClient, inputModel *SortInputModel) (itemList []sortLlmServerItem, err error) {
...
func bEntryListSortedLlmServer(itemList []sortLlmServerItem, inputModel *SortInputModel) (bsorted bool) {
...
func moveAfterLlmServer(idbefore, idafter string, c *FortiSDKClient, inputModel *SortInputModel) (err error) {
	path := "/pm/config/device/{device}/vdom/{vdom}/llm/server/"
	path, err = replaceParaWithValue(path, inputModel.URLParams)
...
func sortEntryListLlmServer(itemList []sortLlmServerItem, c *FortiSDKClient, inputModel *SortInputModel) (err error) {
	sortby := inputModel.SortBy
	sortdirection := inputModel.SortDirection
	manual_order := inputModel.ManualOrder
	var targetItemOrder []sortLlmServerItem
	if sortby == "name" {
		if sortdirection == "ascending" {
			...
			curItemMap := make(map[string]sortLlmServerItem)
	...
	for i := 0; i < len(targetItemOrder)-1; i++ {
		err = moveAfterLlmServer(targetItemOrder[i+1].name, targetItemOrder[i].name, c, inputModel)
		if err != nil {
...
// CreateUpdateLlmServerSort API operation for FortiManager to sort the firewall policies.
// Returns error for service API and SDK errors.
func (c *FortiSDKClient) CreateUpdateLlmServerSort(inputModel *SortInputModel) (err error) {
	itemList, err := getEntryListLlmServer(c, inputModel)
	...
	bsorted := bEntryListSortedLlmServer(itemList, inputModel)
	if bsorted == true {
		return
	}

	err = sortEntryListLlmServer(itemList, c, inputModel)
	if err != nil {
...
// ReadLlmServerSort API operation for FortiManager to read the firewall policies sort results
// Returns sort status
// Returns error for service API and SDK errors.
func (c *FortiSDKClient) ReadLlmServerSort(inputModel *SortInputModel) (sorted bool, itemMapList []interface{}, err error) {
	itemList, err := getEntryListLlmServer(c, inputModel)
	if err != nil {

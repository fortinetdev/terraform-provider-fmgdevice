...
// sortUserNacPolicyItem contains the parameters for each item
type sortUserNacPolicyItem struct {
	name string
}

func getEntryListUserNacPolicy(c *FortiSDKClient, inputModel *SortInputModel) (itemList []sortUserNacPolicyItem, err error) {
	...
	if err == nil {
		...
		var members []sortUserNacPolicyItem
		for _, v := range listTmp {
			c := v.(map[string]interface{})

			members = append(members,
				sortUserNacPolicyItem{
...
func bEntryListSortedUserNacPolicy(itemList []sortUserNacPolicyItem, inputModel *SortInputModel) (bsorted bool) {
...
func moveAfterUserNacPolicy(idbefore, idafter string, c *FortiSDKClient, inputModel *SortInputModel) (err error) {
	path := "/pm/config/device/{device}/vdom/{vdom}/user/nac-policy/"
	path, err = replaceParaWithValue(path, inputModel.URLParams)
...
func sortEntryListUserNacPolicy(itemList []sortUserNacPolicyItem, c *FortiSDKClient, inputModel *SortInputModel) (err error) {
	sortby := inputModel.SortBy
	sortdirection := inputModel.SortDirection
	manual_order := inputModel.ManualOrder
	var targetItemOrder []sortUserNacPolicyItem
	if sortby == "name" {
		if sortdirection == "ascending" {
			...
			curItemMap := make(map[string]sortUserNacPolicyItem)
	...
	for i := 0; i < len(targetItemOrder)-1; i++ {
		err = moveAfterUserNacPolicy(targetItemOrder[i+1].name, targetItemOrder[i].name, c, inputModel)
		if err != nil {
...
// CreateUpdateUserNacPolicySort API operation for FortiManager to sort the firewall policies.
// Returns error for service API and SDK errors.
func (c *FortiSDKClient) CreateUpdateUserNacPolicySort(inputModel *SortInputModel) (err error) {
	itemList, err := getEntryListUserNacPolicy(c, inputModel)
	...
	bsorted := bEntryListSortedUserNacPolicy(itemList, inputModel)
	if bsorted == true {
		return
	}

	err = sortEntryListUserNacPolicy(itemList, c, inputModel)
	if err != nil {
...
// ReadUserNacPolicySort API operation for FortiManager to read the firewall policies sort results
// Returns sort status
// Returns error for service API and SDK errors.
func (c *FortiSDKClient) ReadUserNacPolicySort(inputModel *SortInputModel) (sorted bool, itemMapList []interface{}, err error) {
	itemList, err := getEntryListUserNacPolicy(c, inputModel)
	if err != nil {

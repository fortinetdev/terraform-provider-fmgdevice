...
// sortSystemIpamRulesItem contains the parameters for each item
type sortSystemIpamRulesItem struct {
	name string
}

func getEntryListSystemIpamRules(c *FortiSDKClient, inputModel *SortInputModel) (itemList []sortSystemIpamRulesItem, err error) {
	...
	if err == nil {
		...
		var members []sortSystemIpamRulesItem
		for _, v := range listTmp {
			c := v.(map[string]interface{})

			members = append(members,
				sortSystemIpamRulesItem{
...
func bEntryListSortedSystemIpamRules(itemList []sortSystemIpamRulesItem, inputModel *SortInputModel) (bsorted bool) {
...
func moveAfterSystemIpamRules(idbefore, idafter string, c *FortiSDKClient, inputModel *SortInputModel) (err error) {
	path := "/pm/config/device/{device}/global/system/ipam/rules/"
	path, err = replaceParaWithValue(path, inputModel.URLParams)
...
func sortEntryListSystemIpamRules(itemList []sortSystemIpamRulesItem, c *FortiSDKClient, inputModel *SortInputModel) (err error) {
	sortby := inputModel.SortBy
	sortdirection := inputModel.SortDirection
	manual_order := inputModel.ManualOrder
	var targetItemOrder []sortSystemIpamRulesItem
	if sortby == "name" {
		if sortdirection == "ascending" {
			...
			curItemMap := make(map[string]sortSystemIpamRulesItem)
	...
	for i := 0; i < len(targetItemOrder)-1; i++ {
		err = moveAfterSystemIpamRules(targetItemOrder[i+1].name, targetItemOrder[i].name, c, inputModel)
		if err != nil {
...
// CreateUpdateSystemIpamRulesSort API operation for FortiManager to sort the firewall policies.
// Returns error for service API and SDK errors.
func (c *FortiSDKClient) CreateUpdateSystemIpamRulesSort(inputModel *SortInputModel) (err error) {
	itemList, err := getEntryListSystemIpamRules(c, inputModel)
	...
	bsorted := bEntryListSortedSystemIpamRules(itemList, inputModel)
	if bsorted == true {
		return
	}

	err = sortEntryListSystemIpamRules(itemList, c, inputModel)
	if err != nil {
...
// ReadSystemIpamRulesSort API operation for FortiManager to read the firewall policies sort results
// Returns sort status
// Returns error for service API and SDK errors.
func (c *FortiSDKClient) ReadSystemIpamRulesSort(inputModel *SortInputModel) (sorted bool, itemMapList []interface{}, err error) {
	itemList, err := getEntryListSystemIpamRules(c, inputModel)
	if err != nil {

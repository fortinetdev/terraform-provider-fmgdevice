...
// sortSystemSdwanZoneItem contains the parameters for each item
type sortSystemSdwanZoneItem struct {
	name string
}

func getEntryListSystemSdwanZone(c *FortiSDKClient, inputModel *SortInputModel) (itemList []sortSystemSdwanZoneItem, err error) {
	...
	if err == nil {
		...
		var members []sortSystemSdwanZoneItem
		for _, v := range listTmp {
			c := v.(map[string]interface{})

			members = append(members,
				sortSystemSdwanZoneItem{
...
func bEntryListSortedSystemSdwanZone(itemList []sortSystemSdwanZoneItem, inputModel *SortInputModel) (bsorted bool) {
...
func moveAfterSystemSdwanZone(idbefore, idafter string, c *FortiSDKClient, inputModel *SortInputModel) (err error) {
	path := "/pm/config/device/{device}/vdom/{vdom}/system/sdwan/zone/"
	path, err = replaceParaWithValue(path, inputModel.URLParams)
...
func sortEntryListSystemSdwanZone(itemList []sortSystemSdwanZoneItem, c *FortiSDKClient, inputModel *SortInputModel) (err error) {
	sortby := inputModel.SortBy
	sortdirection := inputModel.SortDirection
	manual_order := inputModel.ManualOrder
	var targetItemOrder []sortSystemSdwanZoneItem
	if sortby == "name" {
		if sortdirection == "ascending" {
			...
			curItemMap := make(map[string]sortSystemSdwanZoneItem)
	...
	for i := 0; i < len(targetItemOrder)-1; i++ {
		err = moveAfterSystemSdwanZone(targetItemOrder[i+1].name, targetItemOrder[i].name, c, inputModel)
		if err != nil {
...
// CreateUpdateSystemSdwanZoneSort API operation for FortiManager to sort the firewall policies.
// Returns error for service API and SDK errors.
func (c *FortiSDKClient) CreateUpdateSystemSdwanZoneSort(inputModel *SortInputModel) (err error) {
	itemList, err := getEntryListSystemSdwanZone(c, inputModel)
	...
	bsorted := bEntryListSortedSystemSdwanZone(itemList, inputModel)
	if bsorted == true {
		return
	}

	err = sortEntryListSystemSdwanZone(itemList, c, inputModel)
	if err != nil {
...
// ReadSystemSdwanZoneSort API operation for FortiManager to read the firewall policies sort results
// Returns sort status
// Returns error for service API and SDK errors.
func (c *FortiSDKClient) ReadSystemSdwanZoneSort(inputModel *SortInputModel) (sorted bool, itemMapList []interface{}, err error) {
	itemList, err := getEntryListSystemSdwanZone(c, inputModel)
	if err != nil {

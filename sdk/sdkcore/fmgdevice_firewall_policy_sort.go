...
// sortFirewallPolicyItem contains the parameters for each item
type sortFirewallPolicyItem struct {
	policyid int
}

func getEntryListFirewallPolicy(c *FortiSDKClient, inputModel *SortInputModel) (itemList []sortFirewallPolicyItem, err error) {
	path := "/pm/config/device/{device}/vdom/{vdom}/firewall/policy"
...
func bEntryListSortedFirewallPolicy(itemList []sortFirewallPolicyItem, inputModel *SortInputModel) (bsorted bool) {
...
func moveAfterFirewallPolicy(idbefore, idafter int, c *FortiSDKClient, inputModel *SortInputModel) (err error) {
...
func sortEntryListFirewallPolicy(itemList []sortFirewallPolicyItem, c *FortiSDKClient, inputModel *SortInputModel) (err error) {
	sortby := inputModel.SortBy
	sortdirection := inputModel.SortDirection
	manual_order := inputModel.ManualOrder
	...
	for i := 0; i < len(targetItemOrder)-1; i++ {
		err = moveAfterFirewallPolicy(targetItemOrder[i+1].policyid, targetItemOrder[i].policyid, c, inputModel)
		if err != nil {
...
// CreateUpdateFirewallPolicySort API operation for FortiManager to sort the firewall policies.
// Returns error for service API and SDK errors.
func (c *FortiSDKClient) CreateUpdateFirewallPolicySort(inputModel *SortInputModel) (err error) {
	itemList, err := getEntryListFirewallPolicy(c, inputModel)
	...
	bsorted := bEntryListSortedFirewallPolicy(itemList, inputModel)
	if bsorted == true {
		return
	}

	err = sortEntryListFirewallPolicy(itemList, c, inputModel)
	if err != nil {
...
// ReadFirewallPolicySort API operation for FortiManager to read the firewall policies sort results
// Returns sort status
// Returns error for service API and SDK errors.
func (c *FortiSDKClient) ReadFirewallPolicySort(inputModel *SortInputModel) (sorted bool, itemMapList []interface{}, err error) {
	itemList, err := getEntryListFirewallPolicy(c, inputModel)
	if err != nil {

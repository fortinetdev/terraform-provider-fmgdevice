...
// sortFirewallDosPolicy6Item contains the parameters for each item
type sortFirewallDosPolicy6Item struct {
	policyid int
}

func getEntryListFirewallDosPolicy6(c *FortiSDKClient, inputModel *SortInputModel) (itemList []sortFirewallDosPolicy6Item, err error) {
	path := "/pm/config/device/{device}/vdom/{vdom}/firewall/DoS-policy6"
	...
	if err == nil {
		...
		var members []sortFirewallDosPolicy6Item
		for _, v := range listTmp {
			c := v.(map[string]interface{})

			members = append(members,
				sortFirewallDosPolicy6Item{
...
func bEntryListSortedFirewallDosPolicy6(itemList []sortFirewallDosPolicy6Item, inputModel *SortInputModel) (bsorted bool) {
...
func moveAfterFirewallDosPolicy6(idbefore, idafter int, c *FortiSDKClient, inputModel *SortInputModel) (err error) {
...
func sortEntryListFirewallDosPolicy6(itemList []sortFirewallDosPolicy6Item, c *FortiSDKClient, inputModel *SortInputModel) (err error) {
	sortby := inputModel.SortBy
	sortdirection := inputModel.SortDirection
	manual_order := inputModel.ManualOrder
	var targetItemOrder []sortFirewallDosPolicy6Item
	if sortby == "policyid" {
		if sortdirection == "ascending" {
			...
			curItemMap := make(map[string]sortFirewallDosPolicy6Item)
	...
	for i := 0; i < len(targetItemOrder)-1; i++ {
		err = moveAfterFirewallDosPolicy6(targetItemOrder[i+1].policyid, targetItemOrder[i].policyid, c, inputModel)
		if err != nil {
...
// CreateUpdateFirewallDosPolicy6Sort API operation for FortiManager to sort the firewall policies.
// Returns error for service API and SDK errors.
func (c *FortiSDKClient) CreateUpdateFirewallDosPolicy6Sort(inputModel *SortInputModel) (err error) {
	itemList, err := getEntryListFirewallDosPolicy6(c, inputModel)
	...
	bsorted := bEntryListSortedFirewallDosPolicy6(itemList, inputModel)
	if bsorted == true {
		return
	}

	err = sortEntryListFirewallDosPolicy6(itemList, c, inputModel)
	if err != nil {
...
// ReadFirewallDosPolicy6Sort API operation for FortiManager to read the firewall policies sort results
// Returns sort status
// Returns error for service API and SDK errors.
func (c *FortiSDKClient) ReadFirewallDosPolicy6Sort(inputModel *SortInputModel) (sorted bool, itemMapList []interface{}, err error) {
	itemList, err := getEntryListFirewallDosPolicy6(c, inputModel)
	if err != nil {

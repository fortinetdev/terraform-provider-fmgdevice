...
func resourceWebProxyRedirectProfileEntriesSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceWebProxyRedirectProfileEntriesSortCreateUpdate,
		Read:   resourceWebProxyRedirectProfileEntriesSortRead,
		Update: resourceWebProxyRedirectProfileEntriesSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceWebProxyRedirectProfileEntriesSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateWebProxyRedirectProfileEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting WebProxyRedirectProfileEntries: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceWebProxyRedirectProfileEntriesSortRead(d, m)
}

func resourceWebProxyRedirectProfileEntriesSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadWebProxyRedirectProfileEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading WebProxyRedirectProfileEntries sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading WebProxyRedirectProfileEntries List for (%s): %s", d.Id(), err)
		}
	} else {

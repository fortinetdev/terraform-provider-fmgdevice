...
func resourceSystemHealthCheckFortiguardSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceSystemHealthCheckFortiguardSortCreateUpdate,
		Read:   resourceSystemHealthCheckFortiguardSortRead,
		Update: resourceSystemHealthCheckFortiguardSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceSystemHealthCheckFortiguardSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateSystemHealthCheckFortiguardSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting SystemHealthCheckFortiguard: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceSystemHealthCheckFortiguardSortRead(d, m)
}

func resourceSystemHealthCheckFortiguardSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadSystemHealthCheckFortiguardSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading SystemHealthCheckFortiguard sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading SystemHealthCheckFortiguard List for (%s): %s", d.Id(), err)
		}
	} else {

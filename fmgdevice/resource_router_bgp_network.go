...
func resourceRouterBgpNetworkCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateRouterBgpNetwork(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		v, err := c.CreateRouterBgpNetwork(obj, paradict, wsParams)
		if err != nil {
...
func resourceRouterBgpNetworkDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteRouterBgpNetwork(mkey, paradict, wsParams)
	if err != nil {

...
func resourceWirelessControllerHotspot20AnqpVenueNameCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		// check existing
		o, err = c.ReadWirelessControllerHotspot20AnqpVenueName(mkey, paradict)
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateWirelessControllerHotspot20AnqpVenueName(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateWirelessControllerHotspot20AnqpVenueName(obj, paradict, wsParams)
		if err != nil {
...
func resourceWirelessControllerHotspot20AnqpVenueNameUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateWirelessControllerHotspot20AnqpVenueName(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating WirelessControllerHotspot20AnqpVenueName resource: %v", err)
	}
...
func resourceWirelessControllerHotspot20AnqpVenueNameDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteWirelessControllerHotspot20AnqpVenueName(mkey, paradict, wsParams)
	if err != nil {
...
func flattenWirelessControllerHotspot20AnqpVenueNameValueList(v interface{}, d *schema.ResourceData, pre string) []map[string]interface{} {
	...
	for _, r := range l {
		...
		if _, ok := i["index"]; ok {
			v := flattenWirelessControllerHotspot20AnqpVenueNameValueListIndex(i["index"], d, pre_append)
			tmp["index"] = fortiAPISubPartPatch(v, "WirelessControllerHotspot20AnqpVenueName-ValueList-Index")
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "lang"
		if _, ok := i["lang"]; ok {
			v := flattenWirelessControllerHotspot20AnqpVenueNameValueListLang(i["lang"], d, pre_append)
			tmp["lang"] = fortiAPISubPartPatch(v, "WirelessControllerHotspot20AnqpVenueName-ValueList-Lang")
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "value"
		if _, ok := i["value"]; ok {
			v := flattenWirelessControllerHotspot20AnqpVenueNameValueListValue(i["value"], d, pre_append)
			tmp["value"] = fortiAPISubPartPatch(v, "WirelessControllerHotspot20AnqpVenueName-ValueList-Value")
		}
...
func refreshObjectWirelessControllerHotspot20AnqpVenueName(d *schema.ResourceData, o map[string]interface{}) error {
	...
	if err = d.Set("name", flattenWirelessControllerHotspot20AnqpVenueNameName(o["name"], d, "name")); err != nil {
		if vv, ok := fortiAPIPatch(o["name"], "WirelessControllerHotspot20AnqpVenueName-Name"); ok {
	...
	if isImportTable() {
		if err = d.Set("value_list", flattenWirelessControllerHotspot20AnqpVenueNameValueList(o["value-list"], d, "value_list")); err != nil {
			if vv, ok := fortiAPIPatch(o["value-list"], "WirelessControllerHotspot20AnqpVenueName-ValueList"); ok {

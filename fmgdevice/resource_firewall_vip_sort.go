...
func resourceFirewallVipSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceFirewallVipSortCreateUpdate,
		Read:   resourceFirewallVipSortRead,
		Update: resourceFirewallVipSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceFirewallVipSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateFirewallVipSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting FirewallVip: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceFirewallVipSortRead(d, m)
}

func resourceFirewallVipSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadFirewallVipSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading FirewallVip sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading FirewallVip List for (%s): %s", d.Id(), err)
		}
	} else {

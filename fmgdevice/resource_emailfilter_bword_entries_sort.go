...
func resourceEmailfilterBwordEntriesSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceEmailfilterBwordEntriesSortCreateUpdate,
		Read:   resourceEmailfilterBwordEntriesSortRead,
		Update: resourceEmailfilterBwordEntriesSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceEmailfilterBwordEntriesSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateEmailfilterBwordEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting EmailfilterBwordEntries: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceEmailfilterBwordEntriesSortRead(d, m)
}

func resourceEmailfilterBwordEntriesSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadEmailfilterBwordEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading EmailfilterBwordEntries sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading EmailfilterBwordEntries List for (%s): %s", d.Id(), err)
		}
	} else {

...
func resourceWebProxyDebugUrlCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateWebProxyDebugUrl(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateWebProxyDebugUrl(obj, paradict, wsParams)
		if err != nil {
...
func resourceWebProxyDebugUrlUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateWebProxyDebugUrl(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating WebProxyDebugUrl resource: %v", err)
	}
...
func resourceWebProxyDebugUrlDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteWebProxyDebugUrl(mkey, paradict, wsParams)
	if err != nil {

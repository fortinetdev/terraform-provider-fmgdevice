...
func resourceWirelessControllerQosProfileCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateWirelessControllerQosProfile(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateWirelessControllerQosProfile(obj, paradict, wsParams)
		if err != nil {
...
func resourceWirelessControllerQosProfileUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateWirelessControllerQosProfile(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating WirelessControllerQosProfile resource: %v", err)
	}
...
func resourceWirelessControllerQosProfileDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteWirelessControllerQosProfile(mkey, paradict, wsParams)
	if err != nil {

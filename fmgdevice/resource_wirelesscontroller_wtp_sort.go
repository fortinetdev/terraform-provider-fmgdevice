...
func resourceWirelessControllerWtpSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceWirelessControllerWtpSortCreateUpdate,
		Read:   resourceWirelessControllerWtpSortRead,
		Update: resourceWirelessControllerWtpSortCreateUpdate,
		Delete: schema.Noop,
		...
		Schema: map[string]*schema.Schema{
			...
			"sortby": &schema.Schema{
				...
				ValidateFunc: func(val interface{}, key string) (warns []string, errs []error) {
					v := val.(string)
					availableOptions := []string{"wtp-id"}
					var validValue bool
					...
					if !validValue {
						errs = append(errs, fmt.Errorf("%q must be one of the option of [\"wtp-id\"], got: \"%v\"", key, v))
					}
					return
				},
			},
...
func resourceWirelessControllerWtpSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateWirelessControllerWtpSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting WirelessControllerWtp: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceWirelessControllerWtpSortRead(d, m)
}

func resourceWirelessControllerWtpSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadWirelessControllerWtpSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading WirelessControllerWtp sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading WirelessControllerWtp List for (%s): %s", d.Id(), err)
		}
	} else {

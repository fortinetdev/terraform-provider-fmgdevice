...
func resourceRouterPolicy6Sort() *schema.Resource {
	return &schema.Resource{
		Create: resourceRouterPolicy6SortCreateUpdate,
		Read:   resourceRouterPolicy6SortRead,
		Update: resourceRouterPolicy6SortCreateUpdate,
		Delete: schema.Noop,
...
func resourceRouterPolicy6SortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateRouterPolicy6Sort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting RouterPolicy6: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceRouterPolicy6SortRead(d, m)
}

func resourceRouterPolicy6SortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadRouterPolicy6Sort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading RouterPolicy6 sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading RouterPolicy6 List for (%s): %s", d.Id(), err)
		}
	} else {

...
func resourceSystemSdnConnectorCompartmentListSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceSystemSdnConnectorCompartmentListSortCreateUpdate,
		Read:   resourceSystemSdnConnectorCompartmentListSortRead,
		Update: resourceSystemSdnConnectorCompartmentListSortCreateUpdate,
		Delete: schema.Noop,
		...
		Schema: map[string]*schema.Schema{
			...
			"sortby": &schema.Schema{
				...
				ValidateFunc: func(val interface{}, key string) (warns []string, errs []error) {
					v := val.(string)
					availableOptions := []string{"compartment-id"}
					var validValue bool
					...
					if !validValue {
						errs = append(errs, fmt.Errorf("%q must be one of the option of [\"compartment-id\"], got: \"%v\"", key, v))
					}
					return
				},
			},
...
func resourceSystemSdnConnectorCompartmentListSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateSystemSdnConnectorCompartmentListSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting SystemSdnConnectorCompartmentList: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceSystemSdnConnectorCompartmentListSortRead(d, m)
}

func resourceSystemSdnConnectorCompartmentListSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadSystemSdnConnectorCompartmentListSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading SystemSdnConnectorCompartmentList sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading SystemSdnConnectorCompartmentList List for (%s): %s", d.Id(), err)
		}
	} else {

...
func resourceCasbProfileSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceCasbProfileSortCreateUpdate,
		Read:   resourceCasbProfileSortRead,
		Update: resourceCasbProfileSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceCasbProfileSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateCasbProfileSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting CasbProfile: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceCasbProfileSortRead(d, m)
}

func resourceCasbProfileSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadCasbProfileSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading CasbProfile sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading CasbProfile List for (%s): %s", d.Id(), err)
		}
	} else {

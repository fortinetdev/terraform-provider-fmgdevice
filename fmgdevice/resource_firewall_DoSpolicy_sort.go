...
func resourceFirewallDosPolicySort() *schema.Resource {
	return &schema.Resource{
		Create: resourceFirewallDosPolicySortCreateUpdate,
		Read:   resourceFirewallDosPolicySortRead,
		Update: resourceFirewallDosPolicySortCreateUpdate,
		Delete: schema.Noop,
...
func resourceFirewallDosPolicySortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateFirewallDosPolicySort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting FirewallDosPolicy: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceFirewallDosPolicySortRead(d, m)
}

func resourceFirewallDosPolicySortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadFirewallDosPolicySort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading FirewallDosPolicy sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading FirewallDosPolicy List for (%s): %s", d.Id(), err)
		}
	} else {

...
// Description: <i>This object will be purged after policy copy and install.</i> Configure peer groups.
...
func resourceUserPeergrpCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		// check existing
		o, err = c.ReadUserPeergrp(mkey, paradict)
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateUserPeergrp(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateUserPeergrp(obj, paradict, wsParams)
		if err != nil {
...
func resourceUserPeergrpUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateUserPeergrp(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating UserPeergrp resource: %v", err)
	}
...
func resourceUserPeergrpDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteUserPeergrp(mkey, paradict, wsParams)
	if err != nil {
...
func flattenUserPeergrpMember(v interface{}, d *schema.ResourceData, pre string) interface{} {
...
func refreshObjectUserPeergrp(d *schema.ResourceData, o map[string]interface{}) error {
	...
	if err = d.Set("member", flattenUserPeergrpMember(o["member"], d, "member")); err != nil {
		if vv, ok := fortiAPIPatch(o["member"], "UserPeergrp-Member"); ok {
	...
	if err = d.Set("name", flattenUserPeergrpName(o["name"], d, "name")); err != nil {
		if vv, ok := fortiAPIPatch(o["name"], "UserPeergrp-Name"); ok {

...
func resourceSwitchControllerManagedSwitchSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceSwitchControllerManagedSwitchSortCreateUpdate,
		Read:   resourceSwitchControllerManagedSwitchSortRead,
		Update: resourceSwitchControllerManagedSwitchSortCreateUpdate,
		Delete: schema.Noop,
		...
		Schema: map[string]*schema.Schema{
			...
			"sortby": &schema.Schema{
				...
				ValidateFunc: func(val interface{}, key string) (warns []string, errs []error) {
					v := val.(string)
					availableOptions := []string{"switch-id"}
					var validValue bool
					...
					if !validValue {
						errs = append(errs, fmt.Errorf("%q must be one of the option of [\"switch-id\"], got: \"%v\"", key, v))
					}
					return
				},
			},
...
func resourceSwitchControllerManagedSwitchSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateSwitchControllerManagedSwitchSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting SwitchControllerManagedSwitch: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceSwitchControllerManagedSwitchSortRead(d, m)
}

func resourceSwitchControllerManagedSwitchSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadSwitchControllerManagedSwitchSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading SwitchControllerManagedSwitch sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading SwitchControllerManagedSwitch List for (%s): %s", d.Id(), err)
		}
	} else {

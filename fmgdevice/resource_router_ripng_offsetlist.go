...
func resourceRouterRipngOffsetListCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateRouterRipngOffsetList(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateRouterRipngOffsetList(obj, paradict, wsParams)
		if err != nil {
...
func resourceRouterRipngOffsetListUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateRouterRipngOffsetList(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating RouterRipngOffsetList resource: %v", err)
	}
...
func resourceRouterRipngOffsetListDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteRouterRipngOffsetList(mkey, paradict, wsParams)
	if err != nil {

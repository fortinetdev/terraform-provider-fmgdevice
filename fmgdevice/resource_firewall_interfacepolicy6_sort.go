...
func resourceFirewallInterfacePolicy6Sort() *schema.Resource {
	return &schema.Resource{
		Create: resourceFirewallInterfacePolicy6SortCreateUpdate,
		Read:   resourceFirewallInterfacePolicy6SortRead,
		Update: resourceFirewallInterfacePolicy6SortCreateUpdate,
		Delete: schema.Noop,
...
func resourceFirewallInterfacePolicy6SortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateFirewallInterfacePolicy6Sort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting FirewallInterfacePolicy6: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceFirewallInterfacePolicy6SortRead(d, m)
}

func resourceFirewallInterfacePolicy6SortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadFirewallInterfacePolicy6Sort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading FirewallInterfacePolicy6 sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading FirewallInterfacePolicy6 List for (%s): %s", d.Id(), err)
		}
	} else {

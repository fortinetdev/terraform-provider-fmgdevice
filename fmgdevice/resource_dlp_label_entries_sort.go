...
func resourceDlpLabelEntriesSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceDlpLabelEntriesSortCreateUpdate,
		Read:   resourceDlpLabelEntriesSortRead,
		Update: resourceDlpLabelEntriesSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceDlpLabelEntriesSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateDlpLabelEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting DlpLabelEntries: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceDlpLabelEntriesSortRead(d, m)
}

func resourceDlpLabelEntriesSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadDlpLabelEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading DlpLabelEntries sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading DlpLabelEntries List for (%s): %s", d.Id(), err)
		}
	} else {

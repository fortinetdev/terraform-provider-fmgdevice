...
func resourceSystemIpamPoolsCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateSystemIpamPools(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateSystemIpamPools(obj, paradict, wsParams)
		if err != nil {
...
func resourceSystemIpamPoolsUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateSystemIpamPools(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating SystemIpamPools resource: %v", err)
	}
...
func resourceSystemIpamPoolsDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteSystemIpamPools(mkey, paradict, wsParams)
	if err != nil {

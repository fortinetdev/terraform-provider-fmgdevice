...
func resourceLogAzureSecurityCenter2FilterFreeStyleCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateLogAzureSecurityCenter2FilterFreeStyle(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateLogAzureSecurityCenter2FilterFreeStyle(obj, paradict, wsParams)
		if err != nil {
...
func resourceLogAzureSecurityCenter2FilterFreeStyleUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateLogAzureSecurityCenter2FilterFreeStyle(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating LogAzureSecurityCenter2FilterFreeStyle resource: %v", err)
	}
...
func resourceLogAzureSecurityCenter2FilterFreeStyleDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteLogAzureSecurityCenter2FilterFreeStyle(mkey, paradict, wsParams)
	if err != nil {

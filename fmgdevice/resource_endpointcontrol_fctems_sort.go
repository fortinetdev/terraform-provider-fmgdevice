...
func resourceEndpointControlFctemsSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceEndpointControlFctemsSortCreateUpdate,
		Read:   resourceEndpointControlFctemsSortRead,
		Update: resourceEndpointControlFctemsSortCreateUpdate,
		Delete: schema.Noop,
		...
		Schema: map[string]*schema.Schema{
			...
			"sortby": &schema.Schema{
				...
				ValidateFunc: func(val interface{}, key string) (warns []string, errs []error) {
					v := val.(string)
					availableOptions := []string{"ems-id"}
					var validValue bool
					...
					if !validValue {
						errs = append(errs, fmt.Errorf("%q must be one of the option of [\"ems-id\"], got: \"%v\"", key, v))
					}
					return
				},
			},
...
func resourceEndpointControlFctemsSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateEndpointControlFctemsSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting EndpointControlFctems: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceEndpointControlFctemsSortRead(d, m)
}

func resourceEndpointControlFctemsSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadEndpointControlFctemsSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading EndpointControlFctems sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading EndpointControlFctems List for (%s): %s", d.Id(), err)
		}
	} else {

...
func resourceFirewallPolicySort() *schema.Resource {
	return &schema.Resource{
		Create: resourceFirewallPolicySortCreateUpdate,
		Read:   resourceFirewallPolicySortRead,
		Update: resourceFirewallPolicySortCreateUpdate,
		Delete: schema.Noop,
...
func resourceFirewallPolicySortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateFirewallPolicySort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting FirewallPolicy: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceFirewallPolicySortRead(d, m)
}

func resourceFirewallPolicySortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadFirewallPolicySort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading FirewallPolicy sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading FirewallPolicy List for (%s): %s", d.Id(), err)
		}
	} else {

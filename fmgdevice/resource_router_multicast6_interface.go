...
func resourceRouterMulticast6InterfaceCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateRouterMulticast6Interface(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateRouterMulticast6Interface(obj, paradict, wsParams)
		if err != nil {
...
func resourceRouterMulticast6InterfaceUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateRouterMulticast6Interface(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating RouterMulticast6Interface resource: %v", err)
	}
...
func resourceRouterMulticast6InterfaceDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteRouterMulticast6Interface(mkey, paradict, wsParams)
	if err != nil {

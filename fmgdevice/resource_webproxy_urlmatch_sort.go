...
func resourceWebProxyUrlMatchSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceWebProxyUrlMatchSortCreateUpdate,
		Read:   resourceWebProxyUrlMatchSortRead,
		Update: resourceWebProxyUrlMatchSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceWebProxyUrlMatchSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateWebProxyUrlMatchSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting WebProxyUrlMatch: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceWebProxyUrlMatchSortRead(d, m)
}

func resourceWebProxyUrlMatchSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadWebProxyUrlMatchSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading WebProxyUrlMatch sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading WebProxyUrlMatch List for (%s): %s", d.Id(), err)
		}
	} else {

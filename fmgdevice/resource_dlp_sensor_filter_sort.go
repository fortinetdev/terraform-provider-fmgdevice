...
func resourceDlpSensorFilterSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceDlpSensorFilterSortCreateUpdate,
		Read:   resourceDlpSensorFilterSortRead,
		Update: resourceDlpSensorFilterSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceDlpSensorFilterSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateDlpSensorFilterSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting DlpSensorFilter: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceDlpSensorFilterSortRead(d, m)
}

func resourceDlpSensorFilterSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadDlpSensorFilterSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading DlpSensorFilter sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading DlpSensorFilter List for (%s): %s", d.Id(), err)
		}
	} else {

...
func resourceFirewallSecurityPolicySortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateFirewallSecurityPolicySort(&input_model)
	if err != nil {
...
func resourceFirewallSecurityPolicySortRead(d *schema.ResourceData, m interface{}) error {
	...
	sorted, o, err := c.ReadFirewallSecurityPolicySort(&input_model)
	if err != nil {
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading FirewallSecurityPolicy List for (%s): %s", d.Id(), err)
		}
	} else {

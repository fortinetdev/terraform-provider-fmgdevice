...
func resourceDlpExactDataMatchColumnsSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceDlpExactDataMatchColumnsSortCreateUpdate,
		Read:   resourceDlpExactDataMatchColumnsSortRead,
		Update: resourceDlpExactDataMatchColumnsSortCreateUpdate,
		Delete: schema.Noop,
		...
		Schema: map[string]*schema.Schema{
			...
			"sortby": &schema.Schema{
				...
				ValidateFunc: func(val interface{}, key string) (warns []string, errs []error) {
					...
					if !validValue {
						errs = append(errs, fmt.Errorf("%q must be one of the option of [\"index\"], got: \"%v\"", key, v))
					}
					return
				},
			},
...
func resourceDlpExactDataMatchColumnsSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateDlpExactDataMatchColumnsSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting DlpExactDataMatchColumns: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceDlpExactDataMatchColumnsSortRead(d, m)
}

func resourceDlpExactDataMatchColumnsSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadDlpExactDataMatchColumnsSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading DlpExactDataMatchColumns sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading DlpExactDataMatchColumns List for (%s): %s", d.Id(), err)
		}
	} else {

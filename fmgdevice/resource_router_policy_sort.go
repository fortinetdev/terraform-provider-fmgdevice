...
func resourceRouterPolicySort() *schema.Resource {
	return &schema.Resource{
		Create: resourceRouterPolicySortCreateUpdate,
		Read:   resourceRouterPolicySortRead,
		Update: resourceRouterPolicySortCreateUpdate,
		Delete: schema.Noop,
...
func resourceRouterPolicySortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateRouterPolicySort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting RouterPolicy: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceRouterPolicySortRead(d, m)
}

func resourceRouterPolicySortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadRouterPolicySort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading RouterPolicy sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading RouterPolicy List for (%s): %s", d.Id(), err)
		}
	} else {

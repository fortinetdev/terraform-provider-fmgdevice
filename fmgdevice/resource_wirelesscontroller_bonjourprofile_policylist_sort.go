...
func resourceWirelessControllerBonjourProfilePolicyListSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceWirelessControllerBonjourProfilePolicyListSortCreateUpdate,
		Read:   resourceWirelessControllerBonjourProfilePolicyListSortRead,
		Update: resourceWirelessControllerBonjourProfilePolicyListSortCreateUpdate,
		Delete: schema.Noop,
		...
		Schema: map[string]*schema.Schema{
			...
			"sortby": &schema.Schema{
				...
				ValidateFunc: func(val interface{}, key string) (warns []string, errs []error) {
					v := val.(string)
					availableOptions := []string{"policy-id"}
					var validValue bool
					...
					if !validValue {
						errs = append(errs, fmt.Errorf("%q must be one of the option of [\"policy-id\"], got: \"%v\"", key, v))
					}
					return
				},
			},
...
func resourceWirelessControllerBonjourProfilePolicyListSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateWirelessControllerBonjourProfilePolicyListSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting WirelessControllerBonjourProfilePolicyList: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceWirelessControllerBonjourProfilePolicyListSortRead(d, m)
}

func resourceWirelessControllerBonjourProfilePolicyListSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadWirelessControllerBonjourProfilePolicyListSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading WirelessControllerBonjourProfilePolicyList sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading WirelessControllerBonjourProfilePolicyList List for (%s): %s", d.Id(), err)
		}
	} else {

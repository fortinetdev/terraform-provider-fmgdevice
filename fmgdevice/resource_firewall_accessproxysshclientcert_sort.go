...
func resourceFirewallAccessProxySshClientCertSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceFirewallAccessProxySshClientCertSortCreateUpdate,
		Read:   resourceFirewallAccessProxySshClientCertSortRead,
		Update: resourceFirewallAccessProxySshClientCertSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceFirewallAccessProxySshClientCertSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateFirewallAccessProxySshClientCertSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting FirewallAccessProxySshClientCert: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceFirewallAccessProxySshClientCertSortRead(d, m)
}

func resourceFirewallAccessProxySshClientCertSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadFirewallAccessProxySshClientCertSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading FirewallAccessProxySshClientCert sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading FirewallAccessProxySshClientCert List for (%s): %s", d.Id(), err)
		}
	} else {

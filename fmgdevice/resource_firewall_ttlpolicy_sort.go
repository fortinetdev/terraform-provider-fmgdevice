...
func resourceFirewallTtlPolicySort() *schema.Resource {
	return &schema.Resource{
		Create: resourceFirewallTtlPolicySortCreateUpdate,
		Read:   resourceFirewallTtlPolicySortRead,
		Update: resourceFirewallTtlPolicySortCreateUpdate,
		Delete: schema.Noop,
...
func resourceFirewallTtlPolicySortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateFirewallTtlPolicySort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting FirewallTtlPolicy: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceFirewallTtlPolicySortRead(d, m)
}

func resourceFirewallTtlPolicySortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadFirewallTtlPolicySort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading FirewallTtlPolicy sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading FirewallTtlPolicy List for (%s): %s", d.Id(), err)
		}
	} else {

...
func resourceFirewallInterfacePolicySort() *schema.Resource {
	return &schema.Resource{
		Create: resourceFirewallInterfacePolicySortCreateUpdate,
		Read:   resourceFirewallInterfacePolicySortRead,
		Update: resourceFirewallInterfacePolicySortCreateUpdate,
		Delete: schema.Noop,
...
func resourceFirewallInterfacePolicySortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateFirewallInterfacePolicySort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting FirewallInterfacePolicy: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceFirewallInterfacePolicySortRead(d, m)
}

func resourceFirewallInterfacePolicySortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadFirewallInterfacePolicySort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading FirewallInterfacePolicy sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading FirewallInterfacePolicy List for (%s): %s", d.Id(), err)
		}
	} else {

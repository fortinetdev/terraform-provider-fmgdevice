...
func resourceSystemDeviceUpgradeKnownHaMembersCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateSystemDeviceUpgradeKnownHaMembers(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateSystemDeviceUpgradeKnownHaMembers(obj, paradict, wsParams)
		if err != nil {
...
func resourceSystemDeviceUpgradeKnownHaMembersUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateSystemDeviceUpgradeKnownHaMembers(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating SystemDeviceUpgradeKnownHaMembers resource: %v", err)
	}
...
func resourceSystemDeviceUpgradeKnownHaMembersDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteSystemDeviceUpgradeKnownHaMembers(mkey, paradict, wsParams)
	if err != nil {

...
func resourceVideofilterProfileFiltersSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceVideofilterProfileFiltersSortCreateUpdate,
		Read:   resourceVideofilterProfileFiltersSortRead,
		Update: resourceVideofilterProfileFiltersSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceVideofilterProfileFiltersSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateVideofilterProfileFiltersSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting VideofilterProfileFilters: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceVideofilterProfileFiltersSortRead(d, m)
}

func resourceVideofilterProfileFiltersSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadVideofilterProfileFiltersSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading VideofilterProfileFilters sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading VideofilterProfileFilters List for (%s): %s", d.Id(), err)
		}
	} else {

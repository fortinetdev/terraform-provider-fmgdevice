...
func resourceRouterRipNeighborCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateRouterRipNeighbor(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateRouterRipNeighbor(obj, paradict, wsParams)
		if err != nil {
...
func resourceRouterRipNeighborUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateRouterRipNeighbor(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating RouterRipNeighbor resource: %v", err)
	}
...
func resourceRouterRipNeighborDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteRouterRipNeighbor(mkey, paradict, wsParams)
	if err != nil {

...
func resourceDlpDictionaryEntriesSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceDlpDictionaryEntriesSortCreateUpdate,
		Read:   resourceDlpDictionaryEntriesSortRead,
		Update: resourceDlpDictionaryEntriesSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceDlpDictionaryEntriesSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateDlpDictionaryEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting DlpDictionaryEntries: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceDlpDictionaryEntriesSortRead(d, m)
}

func resourceDlpDictionaryEntriesSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadDlpDictionaryEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading DlpDictionaryEntries sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading DlpDictionaryEntries List for (%s): %s", d.Id(), err)
		}
	} else {

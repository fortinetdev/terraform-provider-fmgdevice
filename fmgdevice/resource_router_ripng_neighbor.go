...
func resourceRouterRipngNeighborCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateRouterRipngNeighbor(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateRouterRipngNeighbor(obj, paradict, wsParams)
		if err != nil {
...
func resourceRouterRipngNeighborUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateRouterRipngNeighbor(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating RouterRipngNeighbor resource: %v", err)
	}
...
func resourceRouterRipngNeighborDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteRouterRipngNeighbor(mkey, paradict, wsParams)
	if err != nil {

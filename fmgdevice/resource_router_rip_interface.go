...
func resourceRouterRipInterfaceCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateRouterRipInterface(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateRouterRipInterface(obj, paradict, wsParams)
		if err != nil {
...
func resourceRouterRipInterfaceUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateRouterRipInterface(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating RouterRipInterface resource: %v", err)
	}
...
func resourceRouterRipInterfaceDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteRouterRipInterface(mkey, paradict, wsParams)
	if err != nil {

...
func resourceLogSyslogd4FilterFreeStyleCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateLogSyslogd4FilterFreeStyle(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateLogSyslogd4FilterFreeStyle(obj, paradict, wsParams)
		if err != nil {
...
func resourceLogSyslogd4FilterFreeStyleUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateLogSyslogd4FilterFreeStyle(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating LogSyslogd4FilterFreeStyle resource: %v", err)
	}
...
func resourceLogSyslogd4FilterFreeStyleDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteLogSyslogd4FilterFreeStyle(mkey, paradict, wsParams)
	if err != nil {

...
func resourceSystemSdwanZoneCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateSystemSdwanZone(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateSystemSdwanZone(obj, paradict, wsParams)
		if err != nil {
...
func resourceSystemSdwanZoneUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateSystemSdwanZone(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating SystemSdwanZone resource: %v", err)
	}
...
func resourceSystemSdwanZoneDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteSystemSdwanZone(mkey, paradict, wsParams)
	if err != nil {

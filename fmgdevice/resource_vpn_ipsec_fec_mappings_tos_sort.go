...
func resourceVpnIpsecFecMappingsTosSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceVpnIpsecFecMappingsTosSortCreateUpdate,
		Read:   resourceVpnIpsecFecMappingsTosSortRead,
		Update: resourceVpnIpsecFecMappingsTosSortCreateUpdate,
		Delete: schema.Noop,
		...
		Schema: map[string]*schema.Schema{
			...
			"sortby": &schema.Schema{
				...
				ValidateFunc: func(val interface{}, key string) (warns []string, errs []error) {
					...
					if !validValue {
						errs = append(errs, fmt.Errorf("%q must be one of the option of [\"seqno\"], got: \"%v\"", key, v))
					}
					return
				},
			},
...
func resourceVpnIpsecFecMappingsTosSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateVpnIpsecFecMappingsTosSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting VpnIpsecFecMappingsTos: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceVpnIpsecFecMappingsTosSortRead(d, m)
}

func resourceVpnIpsecFecMappingsTosSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadVpnIpsecFecMappingsTosSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading VpnIpsecFecMappingsTos sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading VpnIpsecFecMappingsTos List for (%s): %s", d.Id(), err)
		}
	} else {

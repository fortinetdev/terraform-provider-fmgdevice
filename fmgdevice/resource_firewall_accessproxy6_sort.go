...
func resourceFirewallAccessProxy6Sort() *schema.Resource {
	return &schema.Resource{
		Create: resourceFirewallAccessProxy6SortCreateUpdate,
		Read:   resourceFirewallAccessProxy6SortRead,
		Update: resourceFirewallAccessProxy6SortCreateUpdate,
		Delete: schema.Noop,
...
func resourceFirewallAccessProxy6SortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateFirewallAccessProxy6Sort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting FirewallAccessProxy6: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceFirewallAccessProxy6SortRead(d, m)
}

func resourceFirewallAccessProxy6SortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadFirewallAccessProxy6Sort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading FirewallAccessProxy6 sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading FirewallAccessProxy6 List for (%s): %s", d.Id(), err)
		}
	} else {

...
func resourceSwitchControllerLocationCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateSwitchControllerLocation(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateSwitchControllerLocation(obj, paradict, wsParams)
		if err != nil {
...
func resourceSwitchControllerLocationUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateSwitchControllerLocation(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating SwitchControllerLocation resource: %v", err)
	}
...
func resourceSwitchControllerLocationDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteSwitchControllerLocation(mkey, paradict, wsParams)
	if err != nil {

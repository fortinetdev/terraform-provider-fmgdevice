...
func resourceFirewallDosPolicy6Sort() *schema.Resource {
	return &schema.Resource{
		Create: resourceFirewallDosPolicy6SortCreateUpdate,
		Read:   resourceFirewallDosPolicy6SortRead,
		Update: resourceFirewallDosPolicy6SortCreateUpdate,
		Delete: schema.Noop,
...
func resourceFirewallDosPolicy6SortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateFirewallDosPolicy6Sort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting FirewallDosPolicy6: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceFirewallDosPolicy6SortRead(d, m)
}

func resourceFirewallDosPolicy6SortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadFirewallDosPolicy6Sort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading FirewallDosPolicy6 sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading FirewallDosPolicy6 List for (%s): %s", d.Id(), err)
		}
	} else {

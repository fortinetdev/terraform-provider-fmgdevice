...
func resourceSystemPppoeInterfaceCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateSystemPppoeInterface(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateSystemPppoeInterface(obj, paradict, wsParams)
		if err != nil {
...
func resourceSystemPppoeInterfaceUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateSystemPppoeInterface(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating SystemPppoeInterface resource: %v", err)
	}
...
func resourceSystemPppoeInterfaceDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteSystemPppoeInterface(mkey, paradict, wsParams)
	if err != nil {

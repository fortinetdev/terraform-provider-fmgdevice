...
func resourceSystemIpamRulesSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceSystemIpamRulesSortCreateUpdate,
		Read:   resourceSystemIpamRulesSortRead,
		Update: resourceSystemIpamRulesSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceSystemIpamRulesSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateSystemIpamRulesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting SystemIpamRules: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceSystemIpamRulesSortRead(d, m)
}

func resourceSystemIpamRulesSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadSystemIpamRulesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading SystemIpamRules sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading SystemIpamRules List for (%s): %s", d.Id(), err)
		}
	} else {

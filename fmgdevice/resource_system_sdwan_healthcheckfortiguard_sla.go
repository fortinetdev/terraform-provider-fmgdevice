...
func resourceSystemSdwanHealthCheckFortiguardSlaCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateSystemSdwanHealthCheckFortiguardSla(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateSystemSdwanHealthCheckFortiguardSla(obj, paradict, wsParams)
		if err != nil {
...
func resourceSystemSdwanHealthCheckFortiguardSlaUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateSystemSdwanHealthCheckFortiguardSla(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating SystemSdwanHealthCheckFortiguardSla resource: %v", err)
	}
...
func resourceSystemSdwanHealthCheckFortiguardSlaDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteSystemSdwanHealthCheckFortiguardSla(mkey, paradict, wsParams)
	if err != nil {

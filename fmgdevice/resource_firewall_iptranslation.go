...
func resourceFirewallIpTranslationCreate(d *schema.ResourceData, m interface{}) error {
	...
	mkey_tf, mkey_ok := d.GetOk("transid")
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateFirewallIpTranslation(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateFirewallIpTranslation(obj, paradict, wsParams)
		if err != nil {
...
func resourceFirewallIpTranslationUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateFirewallIpTranslation(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating FirewallIpTranslation resource: %v", err)
	}
...
func resourceFirewallIpTranslationDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteFirewallIpTranslation(mkey, paradict, wsParams)
	if err != nil {

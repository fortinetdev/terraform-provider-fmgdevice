...
func resourceSystemAffinityInterruptCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateSystemAffinityInterrupt(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateSystemAffinityInterrupt(obj, paradict, wsParams)
		if err != nil {
...
func resourceSystemAffinityInterruptUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateSystemAffinityInterrupt(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating SystemAffinityInterrupt resource: %v", err)
	}
...
func resourceSystemAffinityInterruptDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteSystemAffinityInterrupt(mkey, paradict, wsParams)
	if err != nil {

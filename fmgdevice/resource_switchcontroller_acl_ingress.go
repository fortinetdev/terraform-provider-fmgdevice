...
func resourceSwitchControllerAclIngressCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateSwitchControllerAclIngress(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateSwitchControllerAclIngress(obj, paradict, wsParams)
		if err != nil {
...
func resourceSwitchControllerAclIngressUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateSwitchControllerAclIngress(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating SwitchControllerAclIngress resource: %v", err)
	}
...
func resourceSwitchControllerAclIngressDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteSwitchControllerAclIngress(mkey, paradict, wsParams)
	if err != nil {

...
func resourceFirewallAccessProxyVirtualHostSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceFirewallAccessProxyVirtualHostSortCreateUpdate,
		Read:   resourceFirewallAccessProxyVirtualHostSortRead,
		Update: resourceFirewallAccessProxyVirtualHostSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceFirewallAccessProxyVirtualHostSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateFirewallAccessProxyVirtualHostSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting FirewallAccessProxyVirtualHost: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceFirewallAccessProxyVirtualHostSortRead(d, m)
}

func resourceFirewallAccessProxyVirtualHostSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadFirewallAccessProxyVirtualHostSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading FirewallAccessProxyVirtualHost sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading FirewallAccessProxyVirtualHost List for (%s): %s", d.Id(), err)
		}
	} else {

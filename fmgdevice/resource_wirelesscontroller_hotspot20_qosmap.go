...
func resourceWirelessControllerHotspot20QosMapCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateWirelessControllerHotspot20QosMap(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateWirelessControllerHotspot20QosMap(obj, paradict, wsParams)
		if err != nil {
...
func resourceWirelessControllerHotspot20QosMapUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateWirelessControllerHotspot20QosMap(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating WirelessControllerHotspot20QosMap resource: %v", err)
	}
...
func resourceWirelessControllerHotspot20QosMapDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteWirelessControllerHotspot20QosMap(mkey, paradict, wsParams)
	if err != nil {

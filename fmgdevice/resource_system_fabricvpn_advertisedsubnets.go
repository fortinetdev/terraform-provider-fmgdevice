...
func resourceSystemFabricVpnAdvertisedSubnetsCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateSystemFabricVpnAdvertisedSubnets(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateSystemFabricVpnAdvertisedSubnets(obj, paradict, wsParams)
		if err != nil {
...
func resourceSystemFabricVpnAdvertisedSubnetsUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateSystemFabricVpnAdvertisedSubnets(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating SystemFabricVpnAdvertisedSubnets resource: %v", err)
	}
...
func resourceSystemFabricVpnAdvertisedSubnetsDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteSystemFabricVpnAdvertisedSubnets(mkey, paradict, wsParams)
	if err != nil {

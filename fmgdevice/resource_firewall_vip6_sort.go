...
func resourceFirewallVip6Sort() *schema.Resource {
	return &schema.Resource{
		Create: resourceFirewallVip6SortCreateUpdate,
		Read:   resourceFirewallVip6SortRead,
		Update: resourceFirewallVip6SortCreateUpdate,
		Delete: schema.Noop,
...
func resourceFirewallVip6SortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateFirewallVip6Sort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting FirewallVip6: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceFirewallVip6SortRead(d, m)
}

func resourceFirewallVip6SortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadFirewallVip6Sort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading FirewallVip6 sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading FirewallVip6 List for (%s): %s", d.Id(), err)
		}
	} else {

...
func resourceDlpFilepatternEntriesSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceDlpFilepatternEntriesSortCreateUpdate,
		Read:   resourceDlpFilepatternEntriesSortRead,
		Update: resourceDlpFilepatternEntriesSortCreateUpdate,
		Delete: schema.Noop,
		...
		Schema: map[string]*schema.Schema{
			...
			"sortby": &schema.Schema{
				...
				ValidateFunc: func(val interface{}, key string) (warns []string, errs []error) {
					v := val.(string)
					availableOptions := []string{"pattern"}
					var validValue bool
					...
					if !validValue {
						errs = append(errs, fmt.Errorf("%q must be one of the option of [\"pattern\"], got: \"%v\"", key, v))
					}
					return
				},
			},
...
func resourceDlpFilepatternEntriesSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateDlpFilepatternEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting DlpFilepatternEntries: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceDlpFilepatternEntriesSortRead(d, m)
}

func resourceDlpFilepatternEntriesSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadDlpFilepatternEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading DlpFilepatternEntries sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading DlpFilepatternEntries List for (%s): %s", d.Id(), err)
		}
	} else {

...
func resourceLogSyslogd3FilterFreeStyleCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateLogSyslogd3FilterFreeStyle(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateLogSyslogd3FilterFreeStyle(obj, paradict, wsParams)
		if err != nil {
...
func resourceLogSyslogd3FilterFreeStyleUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateLogSyslogd3FilterFreeStyle(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating LogSyslogd3FilterFreeStyle resource: %v", err)
	}
...
func resourceLogSyslogd3FilterFreeStyleDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteLogSyslogd3FilterFreeStyle(mkey, paradict, wsParams)
	if err != nil {

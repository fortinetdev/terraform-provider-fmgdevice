...
func resourceIcapLocalServerIcapServiceSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceIcapLocalServerIcapServiceSortCreateUpdate,
		Read:   resourceIcapLocalServerIcapServiceSortRead,
		Update: resourceIcapLocalServerIcapServiceSortCreateUpdate,
		Delete: schema.Noop,
		...
		Schema: map[string]*schema.Schema{
			...
			"sortby": &schema.Schema{
				...
				ValidateFunc: func(val interface{}, key string) (warns []string, errs []error) {
					v := val.(string)
					availableOptions := []string{"service-id"}
					var validValue bool
					...
					if !validValue {
						errs = append(errs, fmt.Errorf("%q must be one of the option of [\"service-id\"], got: \"%v\"", key, v))
					}
					return
				},
			},
...
func resourceIcapLocalServerIcapServiceSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateIcapLocalServerIcapServiceSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting IcapLocalServerIcapService: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceIcapLocalServerIcapServiceSortRead(d, m)
}

func resourceIcapLocalServerIcapServiceSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadIcapLocalServerIcapServiceSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading IcapLocalServerIcapService sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading IcapLocalServerIcapService List for (%s): %s", d.Id(), err)
		}
	} else {

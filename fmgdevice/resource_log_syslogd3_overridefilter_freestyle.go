...
func resourceLogSyslogd3OverrideFilterFreeStyleCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateLogSyslogd3OverrideFilterFreeStyle(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateLogSyslogd3OverrideFilterFreeStyle(obj, paradict, wsParams)
		if err != nil {
...
func resourceLogSyslogd3OverrideFilterFreeStyleUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateLogSyslogd3OverrideFilterFreeStyle(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating LogSyslogd3OverrideFilterFreeStyle resource: %v", err)
	}
...
func resourceLogSyslogd3OverrideFilterFreeStyleDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteLogSyslogd3OverrideFilterFreeStyle(mkey, paradict, wsParams)
	if err != nil {

...
func resourceFirewallSslServerCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateFirewallSslServer(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateFirewallSslServer(obj, paradict, wsParams)
		if err != nil {
...
func resourceFirewallSslServerUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateFirewallSslServer(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating FirewallSslServer resource: %v", err)
	}
...
func resourceFirewallSslServerDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteFirewallSslServer(mkey, paradict, wsParams)
	if err != nil {

...
func resourceSystemSdwanZoneSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceSystemSdwanZoneSortCreateUpdate,
		Read:   resourceSystemSdwanZoneSortRead,
		Update: resourceSystemSdwanZoneSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceSystemSdwanZoneSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateSystemSdwanZoneSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting SystemSdwanZone: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceSystemSdwanZoneSortRead(d, m)
}

func resourceSystemSdwanZoneSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadSystemSdwanZoneSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading SystemSdwanZone sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading SystemSdwanZone List for (%s): %s", d.Id(), err)
		}
	} else {

...
func resourceWanoptContentDeliveryNetworkRuleRulesCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateWanoptContentDeliveryNetworkRuleRules(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateWanoptContentDeliveryNetworkRuleRules(obj, paradict, wsParams)
		if err != nil {
...
func resourceWanoptContentDeliveryNetworkRuleRulesUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateWanoptContentDeliveryNetworkRuleRules(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating WanoptContentDeliveryNetworkRuleRules resource: %v", err)
	}
...
func resourceWanoptContentDeliveryNetworkRuleRulesDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteWanoptContentDeliveryNetworkRuleRules(mkey, paradict, wsParams)
	if err != nil {

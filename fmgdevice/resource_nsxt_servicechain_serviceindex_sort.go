...
func resourceNsxtServiceChainServiceIndexSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceNsxtServiceChainServiceIndexSortCreateUpdate,
		Read:   resourceNsxtServiceChainServiceIndexSortRead,
		Update: resourceNsxtServiceChainServiceIndexSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceNsxtServiceChainServiceIndexSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateNsxtServiceChainServiceIndexSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting NsxtServiceChainServiceIndex: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceNsxtServiceChainServiceIndexSortRead(d, m)
}

func resourceNsxtServiceChainServiceIndexSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadNsxtServiceChainServiceIndexSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading NsxtServiceChainServiceIndex sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading NsxtServiceChainServiceIndex List for (%s): %s", d.Id(), err)
		}
	} else {

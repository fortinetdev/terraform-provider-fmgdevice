...
func resourceRouterMulticastFlowCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateRouterMulticastFlow(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateRouterMulticastFlow(obj, paradict, wsParams)
		if err != nil {
...
func resourceRouterMulticastFlowUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateRouterMulticastFlow(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating RouterMulticastFlow resource: %v", err)
	}
...
func resourceRouterMulticastFlowDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteRouterMulticastFlow(mkey, paradict, wsParams)
	if err != nil {

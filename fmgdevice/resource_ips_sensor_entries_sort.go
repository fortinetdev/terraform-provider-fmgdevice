...
func resourceIpsSensorEntriesSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceIpsSensorEntriesSortCreateUpdate,
		Read:   resourceIpsSensorEntriesSortRead,
		Update: resourceIpsSensorEntriesSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceIpsSensorEntriesSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateIpsSensorEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting IpsSensorEntries: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceIpsSensorEntriesSortRead(d, m)
}

func resourceIpsSensorEntriesSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadIpsSensorEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading IpsSensorEntries sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading IpsSensorEntries List for (%s): %s", d.Id(), err)
		}
	} else {

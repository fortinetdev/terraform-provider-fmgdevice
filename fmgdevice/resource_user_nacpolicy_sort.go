...
func resourceUserNacPolicySort() *schema.Resource {
	return &schema.Resource{
		Create: resourceUserNacPolicySortCreateUpdate,
		Read:   resourceUserNacPolicySortRead,
		Update: resourceUserNacPolicySortCreateUpdate,
		Delete: schema.Noop,
...
func resourceUserNacPolicySortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateUserNacPolicySort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting UserNacPolicy: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceUserNacPolicySortRead(d, m)
}

func resourceUserNacPolicySortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadUserNacPolicySort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading UserNacPolicy sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading UserNacPolicy List for (%s): %s", d.Id(), err)
		}
	} else {

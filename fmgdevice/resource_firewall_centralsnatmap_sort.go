...
func resourceFirewallCentralSnatMapSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateFirewallCentralSnatMapSort(&input_model)
	if err != nil {
...
func resourceFirewallCentralSnatMapSortRead(d *schema.ResourceData, m interface{}) error {
	...
	sorted, o, err := c.ReadFirewallCentralSnatMapSort(&input_model)
	if err != nil {
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading FirewallCentralSnatMap List for (%s): %s", d.Id(), err)
		}
	} else {

...
func resourceRouterOspfAreaCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateRouterOspfArea(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateRouterOspfArea(obj, paradict, wsParams)
		if err != nil {
...
func resourceRouterOspfAreaUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateRouterOspfArea(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating RouterOspfArea resource: %v", err)
	}
...
func resourceRouterOspfAreaDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteRouterOspfArea(mkey, paradict, wsParams)
	if err != nil {

...
func resourceLogFortiguardFilterFreeStyleCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateLogFortiguardFilterFreeStyle(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateLogFortiguardFilterFreeStyle(obj, paradict, wsParams)
		if err != nil {
...
func resourceLogFortiguardFilterFreeStyleUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateLogFortiguardFilterFreeStyle(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating LogFortiguardFilterFreeStyle resource: %v", err)
	}
...
func resourceLogFortiguardFilterFreeStyleDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteLogFortiguardFilterFreeStyle(mkey, paradict, wsParams)
	if err != nil {

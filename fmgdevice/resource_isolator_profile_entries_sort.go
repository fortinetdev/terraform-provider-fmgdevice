...
func resourceIsolatorProfileEntriesSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceIsolatorProfileEntriesSortCreateUpdate,
		Read:   resourceIsolatorProfileEntriesSortRead,
		Update: resourceIsolatorProfileEntriesSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceIsolatorProfileEntriesSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateIsolatorProfileEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting IsolatorProfileEntries: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceIsolatorProfileEntriesSortRead(d, m)
}

func resourceIsolatorProfileEntriesSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadIsolatorProfileEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading IsolatorProfileEntries sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading IsolatorProfileEntries List for (%s): %s", d.Id(), err)
		}
	} else {

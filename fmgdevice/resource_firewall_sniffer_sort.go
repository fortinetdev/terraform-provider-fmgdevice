...
func resourceFirewallSnifferSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceFirewallSnifferSortCreateUpdate,
		Read:   resourceFirewallSnifferSortRead,
		Update: resourceFirewallSnifferSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceFirewallSnifferSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateFirewallSnifferSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting FirewallSniffer: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceFirewallSnifferSortRead(d, m)
}

func resourceFirewallSnifferSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadFirewallSnifferSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading FirewallSniffer sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading FirewallSniffer List for (%s): %s", d.Id(), err)
		}
	} else {

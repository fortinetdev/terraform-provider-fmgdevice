...
func resourceLogSyslogd2FilterFreeStyleCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateLogSyslogd2FilterFreeStyle(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateLogSyslogd2FilterFreeStyle(obj, paradict, wsParams)
		if err != nil {
...
func resourceLogSyslogd2FilterFreeStyleUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateLogSyslogd2FilterFreeStyle(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating LogSyslogd2FilterFreeStyle resource: %v", err)
	}
...
func resourceLogSyslogd2FilterFreeStyleDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteLogSyslogd2FilterFreeStyle(mkey, paradict, wsParams)
	if err != nil {

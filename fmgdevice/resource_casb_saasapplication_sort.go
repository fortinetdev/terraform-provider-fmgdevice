...
func resourceCasbSaasApplicationSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceCasbSaasApplicationSortCreateUpdate,
		Read:   resourceCasbSaasApplicationSortRead,
		Update: resourceCasbSaasApplicationSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceCasbSaasApplicationSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateCasbSaasApplicationSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting CasbSaasApplication: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceCasbSaasApplicationSortRead(d, m)
}

func resourceCasbSaasApplicationSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadCasbSaasApplicationSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading CasbSaasApplication sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading CasbSaasApplication List for (%s): %s", d.Id(), err)
		}
	} else {

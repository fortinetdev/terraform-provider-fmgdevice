...
func resourceApplicationListEntriesSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceApplicationListEntriesSortCreateUpdate,
		Read:   resourceApplicationListEntriesSortRead,
		Update: resourceApplicationListEntriesSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceApplicationListEntriesSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateApplicationListEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting ApplicationListEntries: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceApplicationListEntriesSortRead(d, m)
}

func resourceApplicationListEntriesSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadApplicationListEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading ApplicationListEntries sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading ApplicationListEntries List for (%s): %s", d.Id(), err)
		}
	} else {

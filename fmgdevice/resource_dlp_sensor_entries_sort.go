...
func resourceDlpSensorEntriesSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceDlpSensorEntriesSortCreateUpdate,
		Read:   resourceDlpSensorEntriesSortRead,
		Update: resourceDlpSensorEntriesSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceDlpSensorEntriesSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateDlpSensorEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting DlpSensorEntries: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceDlpSensorEntriesSortRead(d, m)
}

func resourceDlpSensorEntriesSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadDlpSensorEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading DlpSensorEntries sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading DlpSensorEntries List for (%s): %s", d.Id(), err)
		}
	} else {

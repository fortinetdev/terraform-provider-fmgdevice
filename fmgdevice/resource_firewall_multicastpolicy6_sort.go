...
func resourceFirewallMulticastPolicy6Sort() *schema.Resource {
	return &schema.Resource{
		Create: resourceFirewallMulticastPolicy6SortCreateUpdate,
		Read:   resourceFirewallMulticastPolicy6SortRead,
		Update: resourceFirewallMulticastPolicy6SortCreateUpdate,
		Delete: schema.Noop,
...
func resourceFirewallMulticastPolicy6SortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateFirewallMulticastPolicy6Sort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting FirewallMulticastPolicy6: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceFirewallMulticastPolicy6SortRead(d, m)
}

func resourceFirewallMulticastPolicy6SortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadFirewallMulticastPolicy6Sort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading FirewallMulticastPolicy6 sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading FirewallMulticastPolicy6 List for (%s): %s", d.Id(), err)
		}
	} else {

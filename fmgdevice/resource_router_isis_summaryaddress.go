...
func resourceRouterIsisSummaryAddressCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateRouterIsisSummaryAddress(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateRouterIsisSummaryAddress(obj, paradict, wsParams)
		if err != nil {
...
func resourceRouterIsisSummaryAddressUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateRouterIsisSummaryAddress(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating RouterIsisSummaryAddress resource: %v", err)
	}
...
func resourceRouterIsisSummaryAddressDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteRouterIsisSummaryAddress(mkey, paradict, wsParams)
	if err != nil {

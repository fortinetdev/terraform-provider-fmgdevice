...
func resourceFirewallShapingPolicySortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateFirewallShapingPolicySort(&input_model)
	if err != nil {
...
func resourceFirewallShapingPolicySortRead(d *schema.ResourceData, m interface{}) error {
	...
	sorted, o, err := c.ReadFirewallShapingPolicySort(&input_model)
	if err != nil {
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading FirewallShapingPolicy List for (%s): %s", d.Id(), err)
		}
	} else {

...
func resourceSystemSdwanMembersSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceSystemSdwanMembersSortCreateUpdate,
		Read:   resourceSystemSdwanMembersSortRead,
		Update: resourceSystemSdwanMembersSortCreateUpdate,
		Delete: schema.Noop,
		...
		Schema: map[string]*schema.Schema{
			...
			"sortby": &schema.Schema{
				...
				ValidateFunc: func(val interface{}, key string) (warns []string, errs []error) {
					v := val.(string)
					availableOptions := []string{"seq-num"}
					var validValue bool
					...
					if !validValue {
						errs = append(errs, fmt.Errorf("%q must be one of the option of [\"seq-num\"], got: \"%v\"", key, v))
					}
					return
				},
			},
...
func resourceSystemSdwanMembersSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateSystemSdwanMembersSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting SystemSdwanMembers: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceSystemSdwanMembersSortRead(d, m)
}

func resourceSystemSdwanMembersSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadSystemSdwanMembersSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading SystemSdwanMembers sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading SystemSdwanMembers List for (%s): %s", d.Id(), err)
		}
	} else {

...
func resourceCasbUserActivitySort() *schema.Resource {
	return &schema.Resource{
		Create: resourceCasbUserActivitySortCreateUpdate,
		Read:   resourceCasbUserActivitySortRead,
		Update: resourceCasbUserActivitySortCreateUpdate,
		Delete: schema.Noop,
...
func resourceCasbUserActivitySortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateCasbUserActivitySort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting CasbUserActivity: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceCasbUserActivitySortRead(d, m)
}

func resourceCasbUserActivitySortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadCasbUserActivitySort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading CasbUserActivity sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading CasbUserActivity List for (%s): %s", d.Id(), err)
		}
	} else {

...
func resourceWirelessControllerMpskProfileCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateWirelessControllerMpskProfile(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateWirelessControllerMpskProfile(obj, paradict, wsParams)
		if err != nil {
...
func resourceWirelessControllerMpskProfileUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateWirelessControllerMpskProfile(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating WirelessControllerMpskProfile resource: %v", err)
	}
...
func resourceWirelessControllerMpskProfileDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteWirelessControllerMpskProfile(mkey, paradict, wsParams)
	if err != nil {

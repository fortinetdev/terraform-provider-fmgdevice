...
func resourceFirewallAccessProxySort() *schema.Resource {
	return &schema.Resource{
		Create: resourceFirewallAccessProxySortCreateUpdate,
		Read:   resourceFirewallAccessProxySortRead,
		Update: resourceFirewallAccessProxySortCreateUpdate,
		Delete: schema.Noop,
...
func resourceFirewallAccessProxySortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateFirewallAccessProxySort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting FirewallAccessProxy: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceFirewallAccessProxySortRead(d, m)
}

func resourceFirewallAccessProxySortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadFirewallAccessProxySort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading FirewallAccessProxy sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading FirewallAccessProxy List for (%s): %s", d.Id(), err)
		}
	} else {

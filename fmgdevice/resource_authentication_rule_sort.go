...
func resourceAuthenticationRuleSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceAuthenticationRuleSortCreateUpdate,
		Read:   resourceAuthenticationRuleSortRead,
		Update: resourceAuthenticationRuleSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceAuthenticationRuleSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateAuthenticationRuleSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting AuthenticationRule: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceAuthenticationRuleSortRead(d, m)
}

func resourceAuthenticationRuleSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadAuthenticationRuleSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading AuthenticationRule sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading AuthenticationRule List for (%s): %s", d.Id(), err)
		}
	} else {

...
func resourceSystemExternalResourceSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceSystemExternalResourceSortCreateUpdate,
		Read:   resourceSystemExternalResourceSortRead,
		Update: resourceSystemExternalResourceSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceSystemExternalResourceSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateSystemExternalResourceSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting SystemExternalResource: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceSystemExternalResourceSortRead(d, m)
}

func resourceSystemExternalResourceSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadSystemExternalResourceSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading SystemExternalResource sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading SystemExternalResource List for (%s): %s", d.Id(), err)
		}
	} else {

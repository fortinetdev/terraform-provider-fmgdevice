...
func resourceReportLayoutBodyItemSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceReportLayoutBodyItemSortCreateUpdate,
		Read:   resourceReportLayoutBodyItemSortRead,
		Update: resourceReportLayoutBodyItemSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceReportLayoutBodyItemSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateReportLayoutBodyItemSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting ReportLayoutBodyItem: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceReportLayoutBodyItemSortRead(d, m)
}

func resourceReportLayoutBodyItemSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadReportLayoutBodyItemSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading ReportLayoutBodyItem sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading ReportLayoutBodyItem List for (%s): %s", d.Id(), err)
		}
	} else {

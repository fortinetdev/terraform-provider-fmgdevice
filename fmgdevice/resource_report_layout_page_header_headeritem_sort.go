...
func resourceReportLayoutPageHeaderHeaderItemSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceReportLayoutPageHeaderHeaderItemSortCreateUpdate,
		Read:   resourceReportLayoutPageHeaderHeaderItemSortRead,
		Update: resourceReportLayoutPageHeaderHeaderItemSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceReportLayoutPageHeaderHeaderItemSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateReportLayoutPageHeaderHeaderItemSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting ReportLayoutPageHeaderHeaderItem: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceReportLayoutPageHeaderHeaderItemSortRead(d, m)
}

func resourceReportLayoutPageHeaderHeaderItemSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadReportLayoutPageHeaderHeaderItemSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading ReportLayoutPageHeaderHeaderItem sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading ReportLayoutPageHeaderHeaderItem List for (%s): %s", d.Id(), err)
		}
	} else {

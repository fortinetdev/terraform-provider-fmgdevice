...
func resourceSwitchControllerTrafficPolicyCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateSwitchControllerTrafficPolicy(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateSwitchControllerTrafficPolicy(obj, paradict, wsParams)
		if err != nil {
...
func resourceSwitchControllerTrafficPolicyUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateSwitchControllerTrafficPolicy(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating SwitchControllerTrafficPolicy resource: %v", err)
	}
...
func resourceSwitchControllerTrafficPolicyDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteSwitchControllerTrafficPolicy(mkey, paradict, wsParams)
	if err != nil {

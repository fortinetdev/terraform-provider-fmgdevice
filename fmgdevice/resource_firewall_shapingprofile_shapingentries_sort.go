...
func resourceFirewallShapingProfileShapingEntriesSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceFirewallShapingProfileShapingEntriesSortCreateUpdate,
		Read:   resourceFirewallShapingProfileShapingEntriesSortRead,
		Update: resourceFirewallShapingProfileShapingEntriesSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceFirewallShapingProfileShapingEntriesSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateFirewallShapingProfileShapingEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting FirewallShapingProfileShapingEntries: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceFirewallShapingProfileShapingEntriesSortRead(d, m)
}

func resourceFirewallShapingProfileShapingEntriesSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadFirewallShapingProfileShapingEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading FirewallShapingProfileShapingEntries sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading FirewallShapingProfileShapingEntries List for (%s): %s", d.Id(), err)
		}
	} else {

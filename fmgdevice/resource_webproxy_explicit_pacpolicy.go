...
func resourceWebProxyExplicitPacPolicyCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateWebProxyExplicitPacPolicy(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateWebProxyExplicitPacPolicy(obj, paradict, wsParams)
		if err != nil {
...
func resourceWebProxyExplicitPacPolicyUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateWebProxyExplicitPacPolicy(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating WebProxyExplicitPacPolicy resource: %v", err)
	}
...
func resourceWebProxyExplicitPacPolicyDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteWebProxyExplicitPacPolicy(mkey, paradict, wsParams)
	if err != nil {

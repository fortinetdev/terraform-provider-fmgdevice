...
func resourceFirewallIdentityBasedRouteSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceFirewallIdentityBasedRouteSortCreateUpdate,
		Read:   resourceFirewallIdentityBasedRouteSortRead,
		Update: resourceFirewallIdentityBasedRouteSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceFirewallIdentityBasedRouteSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateFirewallIdentityBasedRouteSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting FirewallIdentityBasedRoute: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceFirewallIdentityBasedRouteSortRead(d, m)
}

func resourceFirewallIdentityBasedRouteSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadFirewallIdentityBasedRouteSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading FirewallIdentityBasedRoute sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading FirewallIdentityBasedRoute List for (%s): %s", d.Id(), err)
		}
	} else {

...
func resourceSystemLldpNetworkPolicyCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateSystemLldpNetworkPolicy(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateSystemLldpNetworkPolicy(obj, paradict, wsParams)
		if err != nil {
...
func resourceSystemLldpNetworkPolicyUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateSystemLldpNetworkPolicy(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating SystemLldpNetworkPolicy resource: %v", err)
	}
...
func resourceSystemLldpNetworkPolicyDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteSystemLldpNetworkPolicy(mkey, paradict, wsParams)
	if err != nil {

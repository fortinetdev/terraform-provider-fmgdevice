...
func resourceFirewallServiceCategorySort() *schema.Resource {
	return &schema.Resource{
		Create: resourceFirewallServiceCategorySortCreateUpdate,
		Read:   resourceFirewallServiceCategorySortRead,
		Update: resourceFirewallServiceCategorySortCreateUpdate,
		Delete: schema.Noop,
...
func resourceFirewallServiceCategorySortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateFirewallServiceCategorySort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting FirewallServiceCategory: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceFirewallServiceCategorySortRead(d, m)
}

func resourceFirewallServiceCategorySortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadFirewallServiceCategorySort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading FirewallServiceCategory sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading FirewallServiceCategory List for (%s): %s", d.Id(), err)
		}
	} else {

...
func resourceFirewallMulticastPolicySort() *schema.Resource {
	return &schema.Resource{
		Create: resourceFirewallMulticastPolicySortCreateUpdate,
		Read:   resourceFirewallMulticastPolicySortRead,
		Update: resourceFirewallMulticastPolicySortCreateUpdate,
		Delete: schema.Noop,
...
func resourceFirewallMulticastPolicySortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateFirewallMulticastPolicySort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting FirewallMulticastPolicy: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceFirewallMulticastPolicySortRead(d, m)
}

func resourceFirewallMulticastPolicySortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadFirewallMulticastPolicySort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading FirewallMulticastPolicy sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading FirewallMulticastPolicy List for (%s): %s", d.Id(), err)
		}
	} else {

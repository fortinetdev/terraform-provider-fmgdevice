...
func resourceSystemHaUnicastPeersCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateSystemHaUnicastPeers(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateSystemHaUnicastPeers(obj, paradict, wsParams)
		if err != nil {
...
func resourceSystemHaUnicastPeersUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateSystemHaUnicastPeers(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating SystemHaUnicastPeers resource: %v", err)
	}
...
func resourceSystemHaUnicastPeersDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteSystemHaUnicastPeers(mkey, paradict, wsParams)
	if err != nil {

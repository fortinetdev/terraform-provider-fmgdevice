...
func resourceSshFilterProfileShellCommandsSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceSshFilterProfileShellCommandsSortCreateUpdate,
		Read:   resourceSshFilterProfileShellCommandsSortRead,
		Update: resourceSshFilterProfileShellCommandsSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceSshFilterProfileShellCommandsSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateSshFilterProfileShellCommandsSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting SshFilterProfileShellCommands: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceSshFilterProfileShellCommandsSortRead(d, m)
}

func resourceSshFilterProfileShellCommandsSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadSshFilterProfileShellCommandsSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading SshFilterProfileShellCommands sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading SshFilterProfileShellCommands List for (%s): %s", d.Id(), err)
		}
	} else {

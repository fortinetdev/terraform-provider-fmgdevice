...
func resourceLlmServerSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceLlmServerSortCreateUpdate,
		Read:   resourceLlmServerSortRead,
		Update: resourceLlmServerSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceLlmServerSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateLlmServerSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting LlmServer: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceLlmServerSortRead(d, m)
}

func resourceLlmServerSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadLlmServerSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading LlmServer sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading LlmServer List for (%s): %s", d.Id(), err)
		}
	} else {

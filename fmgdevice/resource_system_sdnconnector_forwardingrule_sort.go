...
func resourceSystemSdnConnectorForwardingRuleSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceSystemSdnConnectorForwardingRuleSortCreateUpdate,
		Read:   resourceSystemSdnConnectorForwardingRuleSortRead,
		Update: resourceSystemSdnConnectorForwardingRuleSortCreateUpdate,
		Delete: schema.Noop,
		...
		Schema: map[string]*schema.Schema{
			...
			"sortby": &schema.Schema{
				...
				ValidateFunc: func(val interface{}, key string) (warns []string, errs []error) {
					v := val.(string)
					availableOptions := []string{"rule-name"}
					var validValue bool
					...
					if !validValue {
						errs = append(errs, fmt.Errorf("%q must be one of the option of [\"rule-name\"], got: \"%v\"", key, v))
					}
					return
				},
			},
...
func resourceSystemSdnConnectorForwardingRuleSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateSystemSdnConnectorForwardingRuleSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting SystemSdnConnectorForwardingRule: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceSystemSdnConnectorForwardingRuleSortRead(d, m)
}

func resourceSystemSdnConnectorForwardingRuleSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadSystemSdnConnectorForwardingRuleSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading SystemSdnConnectorForwardingRule sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading SystemSdnConnectorForwardingRule List for (%s): %s", d.Id(), err)
		}
	} else {

...
func resourceWirelessControllerNacProfileCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateWirelessControllerNacProfile(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateWirelessControllerNacProfile(obj, paradict, wsParams)
		if err != nil {
...
func resourceWirelessControllerNacProfileUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateWirelessControllerNacProfile(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating WirelessControllerNacProfile resource: %v", err)
	}
...
func resourceWirelessControllerNacProfileDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteWirelessControllerNacProfile(mkey, paradict, wsParams)
	if err != nil {

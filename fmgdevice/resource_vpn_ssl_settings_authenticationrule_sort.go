...
func resourceVpnSslSettingsAuthenticationRuleSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceVpnSslSettingsAuthenticationRuleSortCreateUpdate,
		Read:   resourceVpnSslSettingsAuthenticationRuleSortRead,
		Update: resourceVpnSslSettingsAuthenticationRuleSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceVpnSslSettingsAuthenticationRuleSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateVpnSslSettingsAuthenticationRuleSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting VpnSslSettingsAuthenticationRule: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceVpnSslSettingsAuthenticationRuleSortRead(d, m)
}

func resourceVpnSslSettingsAuthenticationRuleSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadVpnSslSettingsAuthenticationRuleSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading VpnSslSettingsAuthenticationRule sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading VpnSslSettingsAuthenticationRule List for (%s): %s", d.Id(), err)
		}
	} else {

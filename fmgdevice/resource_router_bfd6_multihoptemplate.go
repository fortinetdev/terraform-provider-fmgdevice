...
func resourceRouterBfd6MultihopTemplateCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateRouterBfd6MultihopTemplate(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateRouterBfd6MultihopTemplate(obj, paradict, wsParams)
		if err != nil {
...
func resourceRouterBfd6MultihopTemplateUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateRouterBfd6MultihopTemplate(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating RouterBfd6MultihopTemplate resource: %v", err)
	}
...
func resourceRouterBfd6MultihopTemplateDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteRouterBfd6MultihopTemplate(mkey, paradict, wsParams)
	if err != nil {

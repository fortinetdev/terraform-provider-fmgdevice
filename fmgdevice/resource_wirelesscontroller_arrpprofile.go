...
func resourceWirelessControllerArrpProfileCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateWirelessControllerArrpProfile(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateWirelessControllerArrpProfile(obj, paradict, wsParams)
		if err != nil {
...
func resourceWirelessControllerArrpProfileUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateWirelessControllerArrpProfile(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating WirelessControllerArrpProfile resource: %v", err)
	}
...
func resourceWirelessControllerArrpProfileDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteWirelessControllerArrpProfile(mkey, paradict, wsParams)
	if err != nil {

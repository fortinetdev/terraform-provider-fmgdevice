...
func resourceSystemSdnConnectorExternalAccountListSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceSystemSdnConnectorExternalAccountListSortCreateUpdate,
		Read:   resourceSystemSdnConnectorExternalAccountListSortRead,
		Update: resourceSystemSdnConnectorExternalAccountListSortCreateUpdate,
		Delete: schema.Noop,
		...
		Schema: map[string]*schema.Schema{
			...
			"sortby": &schema.Schema{
				...
				ValidateFunc: func(val interface{}, key string) (warns []string, errs []error) {
					v := val.(string)
					availableOptions := []string{"role-arn"}
					var validValue bool
					...
					if !validValue {
						errs = append(errs, fmt.Errorf("%q must be one of the option of [\"role-arn\"], got: \"%v\"", key, v))
					}
					return
				},
			},
...
func resourceSystemSdnConnectorExternalAccountListSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateSystemSdnConnectorExternalAccountListSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting SystemSdnConnectorExternalAccountList: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceSystemSdnConnectorExternalAccountListSortRead(d, m)
}

func resourceSystemSdnConnectorExternalAccountListSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadSystemSdnConnectorExternalAccountListSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading SystemSdnConnectorExternalAccountList sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading SystemSdnConnectorExternalAccountList List for (%s): %s", d.Id(), err)
		}
	} else {

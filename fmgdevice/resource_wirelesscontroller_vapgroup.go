...
func resourceWirelessControllerVapGroupCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateWirelessControllerVapGroup(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateWirelessControllerVapGroup(obj, paradict, wsParams)
		if err != nil {
...
func resourceWirelessControllerVapGroupUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateWirelessControllerVapGroup(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating WirelessControllerVapGroup resource: %v", err)
	}
...
func resourceWirelessControllerVapGroupDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteWirelessControllerVapGroup(mkey, paradict, wsParams)
	if err != nil {

...
func resourceSystemSdwanServiceSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceSystemSdwanServiceSortCreateUpdate,
		Read:   resourceSystemSdwanServiceSortRead,
		Update: resourceSystemSdwanServiceSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceSystemSdwanServiceSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateSystemSdwanServiceSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting SystemSdwanService: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceSystemSdwanServiceSortRead(d, m)
}

func resourceSystemSdwanServiceSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadSystemSdwanServiceSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading SystemSdwanService sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading SystemSdwanService List for (%s): %s", d.Id(), err)
		}
	} else {

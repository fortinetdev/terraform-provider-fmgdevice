...
func resourceFileFilterProfileRulesSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceFileFilterProfileRulesSortCreateUpdate,
		Read:   resourceFileFilterProfileRulesSortRead,
		Update: resourceFileFilterProfileRulesSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceFileFilterProfileRulesSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateFileFilterProfileRulesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting FileFilterProfileRules: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceFileFilterProfileRulesSortRead(d, m)
}

func resourceFileFilterProfileRulesSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadFileFilterProfileRulesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading FileFilterProfileRules sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading FileFilterProfileRules List for (%s): %s", d.Id(), err)
		}
	} else {

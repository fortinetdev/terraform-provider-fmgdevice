...
func resourceCasbAttributeMatchSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceCasbAttributeMatchSortCreateUpdate,
		Read:   resourceCasbAttributeMatchSortRead,
		Update: resourceCasbAttributeMatchSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceCasbAttributeMatchSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateCasbAttributeMatchSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting CasbAttributeMatch: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceCasbAttributeMatchSortRead(d, m)
}

func resourceCasbAttributeMatchSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadCasbAttributeMatchSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading CasbAttributeMatch sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading CasbAttributeMatch List for (%s): %s", d.Id(), err)
		}
	} else {

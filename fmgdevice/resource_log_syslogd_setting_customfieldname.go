...
func resourceLogSyslogdSettingCustomFieldNameCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateLogSyslogdSettingCustomFieldName(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateLogSyslogdSettingCustomFieldName(obj, paradict, wsParams)
		if err != nil {
...
func resourceLogSyslogdSettingCustomFieldNameUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateLogSyslogdSettingCustomFieldName(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating LogSyslogdSettingCustomFieldName resource: %v", err)
	}
...
func resourceLogSyslogdSettingCustomFieldNameDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteLogSyslogdSettingCustomFieldName(mkey, paradict, wsParams)
	if err != nil {

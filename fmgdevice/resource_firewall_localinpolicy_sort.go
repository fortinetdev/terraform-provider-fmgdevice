...
func resourceFirewallLocalInPolicySort() *schema.Resource {
	return &schema.Resource{
		Create: resourceFirewallLocalInPolicySortCreateUpdate,
		Read:   resourceFirewallLocalInPolicySortRead,
		Update: resourceFirewallLocalInPolicySortCreateUpdate,
		Delete: schema.Noop,
...
func resourceFirewallLocalInPolicySortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateFirewallLocalInPolicySort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting FirewallLocalInPolicy: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceFirewallLocalInPolicySortRead(d, m)
}

func resourceFirewallLocalInPolicySortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadFirewallLocalInPolicySort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading FirewallLocalInPolicy sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading FirewallLocalInPolicy List for (%s): %s", d.Id(), err)
		}
	} else {

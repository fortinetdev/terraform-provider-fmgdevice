...
func resourceSwitchControllerRemoteLogCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateSwitchControllerRemoteLog(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateSwitchControllerRemoteLog(obj, paradict, wsParams)
		if err != nil {
...
func resourceSwitchControllerRemoteLogUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateSwitchControllerRemoteLog(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating SwitchControllerRemoteLog resource: %v", err)
	}
...
func resourceSwitchControllerRemoteLogDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteSwitchControllerRemoteLog(mkey, paradict, wsParams)
	if err != nil {

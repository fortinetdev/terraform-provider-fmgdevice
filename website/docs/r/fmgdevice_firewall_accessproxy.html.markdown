...
layout: "fmgdevice"
page_title: "FortiManager Device: fmgdevice_firewall_accessproxy"
description: |-
  <i>This object will be purged after policy copy and install.</i> Configure IPv4 access proxy.
---

# fmgdevice_firewall_accessproxy
<i>This object will be purged after policy copy and install.</i> Configure IPv4 access proxy.

~> The following variables have sub resource. Avoid using them together, otherwise conflicts and overwrites may occur.
>- `api_gateway`: `fmgdevice_firewall_accessproxy_apigateway`
>- `api_gateway6`: `fmgdevice_firewall_accessproxy_apigateway6`
>- `realservers`: `fmgdevice_firewall_accessproxy_realservers`
>- `server_pubkey_auth_settings`: `fmgdevice_firewall_accessproxy_serverpubkeyauthsettings`
...
* `ldb_method` - Ldb-Method. Valid values: `static`, `round-robin`, `weighted`, `least-session`, `least-rtt`, `first-alive`.
...
* `server_pubkey_auth` - Server-Pubkey-Auth. Valid values: `disable`, `enable`.
...
* `svr_pool_multiplex` - Enable/disable server pool multiplexing (default = disable). Share connected server in HTTP, HTTPS, and web-portal api-gateway. Valid values: `disable`, `enable`.
...
* `ssl_vpn_web_portal` - Agentless VPN web portal.
...
* `permit_agent_forwarding` - Permit-Agent-Forwarding. Valid values: `disable`, `enable`.

* `permit_port_forwarding` - Permit-Port-Forwarding. Valid values: `disable`, `enable`.

* `permit_pty` - Permit-Pty. Valid values: `disable`, `enable`.

* `permit_user_rc` - Permit-User-Rc. Valid values: `disable`, `enable`.

* `permit_x11_forwarding` - Permit-X11-Forwarding. Valid values: `disable`, `enable`.

* `source_address` - Source-Address. Valid values: `disable`, `enable`.


The `cert_extension` block supports:

* `critical` - Critical. Valid values: `no`, `yes`.

* `data` - Data.
* `name` - Name.
* `type` - Type. Valid values: `fixed`, `user`.
...
$ terraform import fmgdevice_firewall_accessproxy.labelname {{name}}

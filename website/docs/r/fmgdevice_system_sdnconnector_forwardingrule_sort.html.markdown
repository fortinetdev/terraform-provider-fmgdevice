...
subcategory: "System"
layout: "fmgdevice"
page_title: "FortiManager Device: fmgdevice_system_sdnconnector_forwardingrule_sort"
description: |-
  Sort for system sdn-connector_forwarding-rule.
---

# fmgdevice_system_sdnconnector_forwardingrule_sort
Sort for system sdn-connector_forwarding-rule.

```hcl
resource "fmgdevice_system_sdnconnector_forwardingrule_sort" "test" {
	sortby        = "rule-name"
...
* `sortby` - (Required) Sort security policies by the value, it currently supports "rule-name".
...
* `sortby` - Sort security policies by the value, it currently supports "rule-name".

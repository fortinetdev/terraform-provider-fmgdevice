...
layout: "fmgdevice"
page_title: "FortiManager Device: fmgdevice_endpointcontrol_fctems_sort"
description: |-
  Sort for endpoint-control fctems.
---

# fmgdevice_endpointcontrol_fctems_sort
Sort for endpoint-control fctems.

```hcl
resource "fmgdevice_endpointcontrol_fctems_sort" "test" {
	sortby        = "ems-id"
...
* `sortby` - (Required) Sort security policies by the value, it currently supports "ems-id".
...
* `sortby` - Sort security policies by the value, it currently supports "ems-id".

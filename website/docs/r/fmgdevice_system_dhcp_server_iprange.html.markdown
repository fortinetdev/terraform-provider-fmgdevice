...
layout: "fmgdevice"
page_title: "FortiManager Device: fmgdevice_system_dhcp_server_iprange"
description: |-
  DHCP IP range configuration.
---

# fmgdevice_system_dhcp_server_iprange
DHCP IP range configuration.

~> This resource is a sub resource for variable `ip_range` of resource `fmgdevice_system_dhcp_server`. Conflict and overwrite may occur if use both of them.
...
System DhcpServerIpRange can be imported using any of these accepted formats:
```
Set import_options = ["device_name=YOUR_VALUE", "device_vdom=YOUR_VALUE", "server=YOUR_VALUE"] in the provider section.

$ export "FORTIMANAGER_IMPORT_TABLE"="true"
$ terraform import fmgdevice_system_dhcp_server_iprange.labelname {{fosid}}

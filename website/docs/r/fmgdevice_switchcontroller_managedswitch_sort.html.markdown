...
layout: "fmgdevice"
page_title: "FortiManager Device: fmgdevice_switchcontroller_managedswitch_sort"
description: |-
  Sort for switch-controller managed-switch.
---

# fmgdevice_switchcontroller_managedswitch_sort
Sort for switch-controller managed-switch.

```hcl
resource "fmgdevice_switchcontroller_managedswitch_sort" "test" {
	sortby        = "switch-id"
...
* `sortby` - (Required) Sort security policies by the value, it currently supports "switch-id".
...
* `sortby` - Sort security policies by the value, it currently supports "switch-id".

...
description: |-
  <i>This object will be purged after policy copy and install.</i> Enable/disable CIFS (Windows sharing) WAN Optimization and configure CIFS WAN Optimization features.
---

# fmgdevice_wanopt_profile_cifs
<i>This object will be purged after policy copy and install.</i> Enable/disable CIFS (Windows sharing) WAN Optimization and configure CIFS WAN Optimization features.

~> This resource is a sub resource for variable `cifs` of resource `fmgdevice_wanopt_profile`. Conflict and overwrite may occur if use both of them.
...
Wanopt ProfileCifs can be imported using any of these accepted formats:
```
Set import_options = ["device_name=YOUR_VALUE", "device_vdom=YOUR_VALUE", "profile=YOUR_VALUE"] in the provider section.

$ export "FORTIMANAGER_IMPORT_TABLE"="true"
$ terraform import fmgdevice_wanopt_profile_cifs.labelname WanoptProfileCifs

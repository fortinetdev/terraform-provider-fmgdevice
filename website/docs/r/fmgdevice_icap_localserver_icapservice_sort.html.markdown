...
subcategory: "ICAP"
layout: "fmgdevice"
page_title: "FortiManager Device: fmgdevice_icap_localserver_icapservice_sort"
description: |-
  Sort for icap local-server_icap-service.
---

# fmgdevice_icap_localserver_icapservice_sort
Sort for icap local-server_icap-service.

```hcl
resource "fmgdevice_icap_localserver_icapservice_sort" "test" {
	sortby        = "service-id"
...
* `sortby` - (Required) Sort security policies by the value, it currently supports "service-id".
...
* `sortby` - Sort security policies by the value, it currently supports "service-id".

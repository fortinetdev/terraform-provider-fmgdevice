...
description: |-
  <i>This object will be purged after policy copy and install.</i> Configure HTTPS options.
---

# fmgdevice_firewall_sslsshprofile_https
<i>This object will be purged after policy copy and install.</i> Configure HTTPS options.

~> This resource is a sub resource for variable `https` of resource `fmgdevice_firewall_sslsshprofile`. Conflict and overwrite may occur if use both of them.
...
Firewall SslSshProfileHttps can be imported using any of these accepted formats:
```
...
$ terraform import fmgdevice_firewall_sslsshprofile_https.labelname FirewallSslSshProfileHttps

...
description: |-
  <i>This object will be purged after policy copy and install.</i> Configure SSL options.
---

# fmgdevice_firewall_sslsshprofile_ssl
<i>This object will be purged after policy copy and install.</i> Configure SSL options.

~> This resource is a sub resource for variable `ssl` of resource `fmgdevice_firewall_sslsshprofile`. Conflict and overwrite may occur if use both of them.
...
Firewall SslSshProfileSsl can be imported using any of these accepted formats:
```
...
$ terraform import fmgdevice_firewall_sslsshprofile_ssl.labelname FirewallSslSshProfileSsl

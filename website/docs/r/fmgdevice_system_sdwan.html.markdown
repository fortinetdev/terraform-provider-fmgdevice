...
# fmgdevice_system_sdwan
Configure redundant Internet connections with multiple outbound links and health-check profiles.

~> The following variables have sub resource. Avoid using them together, otherwise conflicts and overwrites may occur.
>- `duplication`: `fmgdevice_system_sdwan_duplication`
>- `health_check`: `fmgdevice_system_sdwan_healthcheck`
>- `health_check_fortiguard`: `fmgdevice_system_sdwan_healthcheckfortiguard`
>- `members`: `fmgdevice_system_sdwan_members`
>- `neighbor`: `fmgdevice_system_sdwan_neighbor`
>- `service`: `fmgdevice_system_sdwan_service`
>- `zone`: `fmgdevice_system_sdwan_zone`
...
```hcl
resource "fmgdevice_system_sdwan" "trname" {
  app_perf_log_period = 10
  duplication {
    dstaddr               = ["your own value"]
    dstaddr6              = ["your own value"]
    dstintf               = ["your own value"]
    fosid                 = 10
    packet_de_duplication = "disable"
  ...
  duplication_max_num   = 10
  fail_alert_interfaces = ["port2"]
  fail_detect           = "disable"
...
* `ip` - IP/IPv6 address of neighbor or neighbor-group name.
* `member` - Member sequence number list.
...
$ terraform import fmgdevice_system_sdwan.labelname SystemSdwan

...
layout: "fmgdevice"
page_title: "FortiManager Device: fmgdevice_ztna_trafficforwardproxy_sslciphersuites"
description: |-
  SSL/TLS cipher suites acceptable from a client, ordered by priority.
---

# fmgdevice_ztna_trafficforwardproxy_sslciphersuites
SSL/TLS cipher suites acceptable from a client, ordered by priority.

~> This resource is a sub resource for variable `ssl_cipher_suites` of resource `fmgdevice_ztna_trafficforwardproxy`. Conflict and overwrite may occur if use both of them.
...
Ztna TrafficForwardProxySslCipherSuites can be imported using any of these accepted formats:
```
...
$ terraform import fmgdevice_ztna_trafficforwardproxy_sslciphersuites.labelname {{priority}}

...
subcategory: "DLP"
layout: "fmgdevice"
page_title: "FortiManager Device: fmgdevice_dlp_exactdatamatch_columns_sort"
description: |-
  Sort for dlp exact-data-match_columns.
---

# fmgdevice_dlp_exactdatamatch_columns_sort
Sort for dlp exact-data-match_columns.

```hcl
resource "fmgdevice_dlp_exactdatamatch_columns_sort" "test" {
	sortby        = "index"
...
* `sortby` - (Required) Sort security policies by the value, it currently supports "index".
...
* `sortby` - Sort security policies by the value, it currently supports "index".

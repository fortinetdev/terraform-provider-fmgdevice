...
subcategory: "DLP"
layout: "fmgdevice"
page_title: "FortiManager Device: fmgdevice_dlp_filepattern_entries_sort"
description: |-
  Sort for dlp filepattern_entries.
---

# fmgdevice_dlp_filepattern_entries_sort
Sort for dlp filepattern_entries.

```hcl
resource "fmgdevice_dlp_filepattern_entries_sort" "test" {
	sortby        = "pattern"
...
* `sortby` - (Required) Sort security policies by the value, it currently supports "pattern".
...
* `sortby` - Sort security policies by the value, it currently supports "pattern".

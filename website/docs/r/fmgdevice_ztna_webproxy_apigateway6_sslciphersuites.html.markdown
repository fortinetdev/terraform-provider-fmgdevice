...
layout: "fmgdevice"
page_title: "FortiManager Device: fmgdevice_ztna_webproxy_apigateway6_sslciphersuites"
description: |-
  SSL/TLS cipher suites to offer to a server, ordered by priority.
---

# fmgdevice_ztna_webproxy_apigateway6_sslciphersuites
SSL/TLS cipher suites to offer to a server, ordered by priority.

~> This resource is a sub resource for variable `ssl_cipher_suites` of resource `fmgdevice_ztna_webproxy_apigateway6`. Conflict and overwrite may occur if use both of them.
...
Ztna WebProxyApiGateway6SslCipherSuites can be imported using any of these accepted formats:
```
...
$ terraform import fmgdevice_ztna_webproxy_apigateway6_sslciphersuites.labelname {{priority}}

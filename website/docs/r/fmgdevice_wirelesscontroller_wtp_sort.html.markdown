...
layout: "fmgdevice"
page_title: "FortiManager Device: fmgdevice_wirelesscontroller_wtp_sort"
description: |-
  Sort for wireless-controller wtp.
---

# fmgdevice_wirelesscontroller_wtp_sort
Sort for wireless-controller wtp.

```hcl
resource "fmgdevice_wirelesscontroller_wtp_sort" "test" {
	sortby        = "wtp-id"
...
* `sortby` - (Required) Sort security policies by the value, it currently supports "wtp-id".
...
* `sortby` - Sort security policies by the value, it currently supports "wtp-id".
